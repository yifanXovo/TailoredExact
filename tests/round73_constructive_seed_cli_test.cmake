...
set(out "${OUTPUT_ROOT}/round73_seed_cli_${suffix}")
if(EXISTS "${out}")
...
execute_process(COMMAND "${EXACT_EXE}"
  ...
  --method gcap-frontier --algorithm-preset research-round73-vds-joint-seeded-descent
...
if(NOT exit_code EQUAL 0)
  message(FATAL_ERROR "Real Constructive-seeded descent CLI failed: ${exit_code}; artifacts=${out}")
endif()
...
foreach(field strict_certified_original_problem decoded_descent_complete decoded_descent_cross_route_enabled
  ...
  if(NOT value)
    message(FATAL_ERROR "Constructive-seeded descent CLI did not establish ${field}; artifacts=${out}")
  endif()
endforeach()
...
if(NOT mode STREQUAL "decoded-descent-interroute" OR NOT seeds EQUAL 25 OR NOT generations EQUAL 0
    OR upper LESS 0.208333233333 OR upper GREATER 0.208333433334
    OR lower LESS 0.208333233333 OR lower GREATER 0.208333433334)
  message(FATAL_ERROR "Constructive-seeded descent CLI mode/seed/original-objective mismatch; artifacts=${out}")
endif()
file(STRINGS "${out}/hga.csv.joint_seed.csv" constructive_order)
list(LENGTH constructive_order seed_rows)
if(NOT seed_rows EQUAL 4)
  message(FATAL_ERROR "Tiny constructive seed must contain all3 stations")
endif()
file(STRINGS "${out}/external/paper_optimize_ledger.csv" ledger)
list(LENGTH ledger rows)
math(EXPR calls "${rows} - 1")
if(calls LESS 1)
  message(FATAL_ERROR "Constructive-seeded descent CLI never entered the exact backend")
endif()
...
if(NOT starts)
  message(FATAL_ERROR "Constructive-seeded descent CLI did not exercise a native Start decision")
endif()
message(STATUS "Round73ConstructiveSeedCliTests: real CLI certificate5/24; Optimize calls=${calls}; artifacts=${out}")
