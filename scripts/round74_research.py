"""Round74 frozen D7 proof/protection validation on unchanged R73 v6 bytes."""
import json
import os
...
from round73_qualify import ROOT, OUT as ROUND73, sha, write
STAGE=ROOT/'results/unified_exact_round74'
...
def main():
    ...
    qualification = read(ROUND73/'qualification_v6.json')
    ...
    assert read(ROUND73/'runtime_revision/audit.json')['all_checks_passed']
    assert subprocess.check_output(['git','branch','--show-current'],cwd=ROOT).decode().strip()=='codex/round74-jdsx-protection'
    ...
    roles=[(panel['D7'],arm,False) for arm in ['P-GRB','DS-X','JDS-X','K1-R']]
    ...
    for number, (p,arm,forced) in enumerate(roles,1):
        ...
        if arm == 'P-GRB':
            ...
            command += ['--algorithm-preset',{'DS-X':'research-round71-vds-interroute-descent','JDS-X':'research-round73-vds-joint-seeded-descent','K1-R':'research-round65-k1-h'}[arm],
    ...
    identity = dict(source_commit=frozen['source_commit'],binary_sha256=sha(build/'ExactEBRP.exe'),
        source=frozen['source'],qualification_sha256=sha(ROUND73/'qualification_v6.json'),
        ...
        launches=launches,references=references,maximum_process_seconds=4800,formal_performance=True)
    ...
    write(STAGE/'qualification.json',dict(inherited_source=frozen['source_commit'],
        inherited_binary_sha256=identity['binary_sha256'],inherited_ctest_count=54,
    ...
    try:
        for launch in launches:
            ...
            write(OUT/'summary.json',dict(records=records,total_wall_seconds=sum(r['wall_seconds'] for r in records),
                ...
                completed=len(records),planned=4,formal_performance=True))
