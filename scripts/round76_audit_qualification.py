...
from round76_qualify import ROOT,OUT,read,write,sha

def main():
    version=sys.argv[1];started=time.perf_counter()
    destination=OUT/f'qualification_{version}_audit.json'
    assert not destination.exists()
    build=ROOT/'build/round76'/version;q=read(OUT/f'qualification_{version}.json')
    ...
    passed='100% tests passed, 0 tests failed out of 58' in text
    ...
    failed_cli=[]
    for path in build.glob('round76_closure_cli_*/*/result.json'):
        result=read(path)
    ...
    oracle=re.search(r'Round76 composition:.*',text)
    oracle=oracle[0] if oracle else None
    if passed:assert oracle
    report=dict(version=version,passed=passed,tests_total=58,tests_passed=len(re.findall(r'\d+/58 Test .*Passed',text)),
