"""Four confirmation plots from completed audited checkpoints only; no optimizer."""
import hashlib
import json
import math
import time
...
def main():
    ...
    assert len(points)==57 and set(panel)=={'F1','F2','F5','F6'}
    ...
    plt.rcParams.update({'font.family':'DejaVu Sans','font.size':10,
        'svg.fonttype':'none','svg.hashsalt':'round86-frozen-checkpoints'})
    ...
    for identity in ['F1','F2','F5','F6']:
        ...
        fig.suptitle(f'Round86: {identity} comparison at a common {cap}-second cap',fontsize=14,y=.98)
        ...
        fig.text(.075,.11,
            ...
            f'{identity}: V{p["V"]}/M{p["M"]}/Q{p["Q"]}, mathematical T{p["T_seconds"]}; prospective unadapted confirmation. U/L axes use their observed range.',
    ...
    receipt=dict(layout_revision=1,roles=['F1','F2','F5','F6'],data_source='completed_audited_original_campaign',
