"""Offline actual startup/neighborhood/Start audit of the six-role unadapted confirmation runs."""
import csv
import json
import re
import time
...
from round82_analyze import campaign_summary

OUT=ROOT/'results/unified_exact_round82/campaign'
...
def main():
    ...
    for launch,done in zip(frozen['launches'],summary['records']):
        ...
        if arm!='BDS-C':
            ...
            mathematical_zero=row['balanced_physical_audit']['initial']['F']<=1e-7
        ...
        arms.append(row)
    assert len(traces)==6 and controls==12
    output=dict(all_checks_passed=True,arms=arms,start_decisions=starts,controls_checked=controls,
        all25_paths_match_available_prior_startup_screen=None,prior_startup_pairs_checked=0,
        no_prior_routes_consulted=True,actual_Start_decisions=len(starts),
