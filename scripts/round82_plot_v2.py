"""Layout-only second rendering of the same audited checkpoints; first rendering retained."""
import hashlib
import json
import math
import time
...
OUT = ROOT / 'results/unified_exact_round82'
...
def main():
    ...
    destination = OUT / 'figures_v2'
    assert not destination.exists(), 'Do not overwrite a completed plot namespace'
    points = [r for r in read(campaign / 'checkpoints.json') if r['id'] in ['U4', 'U5', 'U6']]
    ...
    plt.rcParams.update({'font.family': 'DejaVu Sans', 'font.size': 10,
                         'svg.fonttype': 'none', 'svg.hashsalt': 'round82-frozen-checkpoints'})
    colors = {'P-GRB': '#575E68', 'BDS-C': '#C45112', 'K1-R': '#2267A5'}
    fig, axes = plt.subplots(1, 3, figsize=(15, 5.4))
    for axis, identity, title in zip(axes, ['U4', 'U5', 'U6'],
                                    ['U4: CitiBike regional, balanced (V30)',
                                     'U5: synthetic moderate (V50)',
                                     'U6: CitiBike compact, shortage (V50)']):
        values = []
        arms = ['P-GRB', 'BDS-C', 'K1-R']
        for arm in arms:
    ...
    fig.suptitle('Round82: unadapted confirmation at common 3600-second caps', fontsize=14, y=0.98)
    ...
    fig.text(0.075, 0.11,
             ...
             'All displayed arms returned normally; elapsed time includes startup and all native calls.\n'
             'Fresh fixed draws from familiar recipes; shared CitiBike station source limits independence.',
    ...
    receipt = dict(layout_revision=2, prior_rendering='figures', data_changed=False, optimizer_calls=0, wall_seconds=time.perf_counter() - started,
