"""Publish existing UTF-8-only preparation commits via nested inline trees, preserving exact hashes."""
import argparse
import base64
import datetime
import hashlib
...
def main():
    ...
    try:
        ...
        for older in OUT.glob('*.json'):
            ...
            if not isinstance(previous,dict):continue  # Non-ledger object readback records are also retained here.
        ...
        def exact_tree(previous,target_tree):
            ...
            if existing is not None:
                ...
                return target_tree
            # Nested file entries on the reachable base tree avoid referencing
            # newly uploaded detached blobs. Only byte-exact UTF-8 text is
            # accepted by this bounded preparation transport variant.
            ...
            for i in range(0,len(parts),2):
                ...
                data=git('cat-file','blob',sha_after);content=data.decode('utf-8')
