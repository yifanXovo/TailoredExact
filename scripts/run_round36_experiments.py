#!/usr/bin/env python3
"""Checksum-resumable serial runner for the frozen Round 36 matrix."""
...
LOCK = common.OUT / ".round36_runner.lock"
SENSITIVE_MARKERS = (
...
def validate_frozen(item: dict[str, Any], manifest: dict[str, Any]) -> None:
    if common.sha256(common.OFFICIAL_MATRIX) != manifest["official_matrix_sha256"]:
        raise RuntimeError("official matrix changed after freeze")
    if common.sha256(common.COMMAND_FREEZE) != manifest["command_freeze_sha256"]:
        raise RuntimeError("command freeze changed after freeze")
    for record in manifest["frozen_artifacts"].values():
        path = common.ROOT / record["path"]
    ...
    if (not common.EXE.is_file() or
            common.sha256(common.EXE) != manifest["gurobi_executable_sha256"]):
...
def acquire_lock() -> None:
    if LOCK.exists():
        try:
            state = common.load_json(LOCK)
            pid = int(state.get("pid", 0))
            if pid > 0:
                os.kill(pid, 0)
                raise RuntimeError("another Round 36 runner is active")
        except OSError:
            pass
...
def result_record(row: dict[str, str], item: dict[str, Any],
    ...
    return {
        "schema": "round36-run-v1", "round_id": 36,
        "run_id": row["run_id"], "serial_order": int(row["serial_order"]),
        "panel_row_id": row["panel_row_id"],
        "instance_id": item["instance_id"],
        "instance_path": item["path"],
        "instance_sha256": item["instance_sha256"],
        "V": item["V"], "M": item["M"], "Q": item["Q"],
        "scenario": item["scenario"], "arm": row["arm"],
...
def run_one(row: dict[str, str], items: dict[str, dict[str, Any]],
    ...
    if directory.exists():
        ...
        common.write_json(target / "invalidation_record.json", {
            "round_id": 36, "run_id": row["run_id"], "reason": reason,
            "algorithmic_solve_state_resumed": False,
        })
    ...
    if result is not None:
        ...
        record.update({
            ...
            "arm_contract_matches":
                result.get("round36_c6_causal_arm") == expected_arm,
            "anchor_safety_valid": result.get("round36_anchor_safety_valid"),
    ...
    if not finalized:
        ...
        raise RuntimeError(
            f"invalid official row: {row['run_id']} rc={return_code} "
    ...
    print(f"DONE {row['serial_order']}/56 {row['run_id']} "
...
def main() -> int:
    ...
    if args.panel_row_id:
        rows = [row for row in rows
                if row["panel_row_id"] in set(args.panel_row_id)]
    ...
    if not common.START_RECORD.is_file():
        common.write_json(common.START_RECORD, {
            "schema": "round36-official-start-v1", "round_id": 36,
