...
ebrp::SolveOptions parseArgs(int argc, char** argv) {
    ...
    if (opt.round37_c6_geometry_policy != "off" &&
        opt.round37_c6_geometry_policy != "pilot-weakest-prefine") {
        throw std::runtime_error(
            "Unsupported --round37-c6-geometry-policy: " +
            opt.round37_c6_geometry_policy);
    }
    if (opt.round37_c6_geometry_policy == "pilot-weakest-prefine" &&
...
ebrp::SolveResult solveGiniFrontierDiagnostic(const ebrp::Instance& instance,
    ...
    if (opt.frontier_execution_mode == "external-gini-tree") {
        ...
        result.notes.push_back(replica_scheduling
            ...
            : (c6_scheduling
                ? "frontier execution mode round31-nonblocking-native-bound: every controlling leaf uses parameter-free next-frontier native-bound targets before lazy child lookahead; target attainment requeues an open leaf and never forces a split"
                : (c5_scheduling
