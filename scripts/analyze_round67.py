...
def analyze():
    ...
    for e in run.runner.entries():
        ...
        for model in model_paths:
            ...
            if e['arm'] in ['LOG','VD-P']:
                ...
                if e['arm']=='LOG':
                    assert not re.search(r'(?m)^\s*state_\d+_\d+\s*$',binary)
                    domains={}
                    for station,value in re.findall(r'(?m)^\s*0 <= state_(\d+)_(\d+) <= 1\s*$',text):
                        domains.setdefault(int(station),set()).add(int(value))
                    assert len(domains)==int(p['V'])
                    expected_codes=set()
                    for station,values in domains.items():
                        assert len(values)==max(values)-min(values)+1
                        expected_codes.update((station,b) for b in range((len(values)-1).bit_length()))
                    actual_codes={(int(i),int(b)) for i,b in re.findall(r'(?m)^\s*state_code_(\d+)_(\d+)\s*$',binary)}
                    assert actual_codes==expected_codes,'code cardinality or singleton mismatch'
                else:
    ...
    pairs=[]
    for c in records:
        if c.get('failure') or c['arm'] not in ['LOG','VD-P']:continue
        for b in records:
            references=['K1-R','P-GRB','VD-P'] if c['arm']=='LOG' else ['K1-R','P-GRB']
    ...
    run.write(run.OUT/'resource_status.json',dict(charged_completed=len(records),performance=sum(r.get('kind')=='performance' for r in records),
        ...
        final_ctest_passed_tests=44,final_ctest_batches=1,ctest_wall_seconds=1.77,preflight_failed_test_attempts=2,
        preflight_failed_ctest_wall_seconds=4.50,
        ctest_note='Separate build qualification; any native unit-test calls are not added to experiment-call count',
