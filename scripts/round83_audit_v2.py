"""Reader v2: preserve explicit untouched empty routes during independent reconstruction."""
import ast
import copy
import json
import math
...
def materialize(witness,key):
    ...
    return dict(routes=[dict(vehicle=k,nodes=[0]+v+[0],operations=[copy.deepcopy(ops[i]) for i in v])
        for k,v in sorted(nodes.items()) if v or not (kind==0 and k==s)],F=witness['F'])
