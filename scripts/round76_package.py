...
from round76_qualify import ROOT,OUT,sha,read,write
...
def main():
    ...
    summary=read(OUT/'startup/summary.json');diagnostic=read(OUT/'diagnostic/summary.json')
    ...
    for version in ['v1']:
        build=ROOT/'build/round76'/version;audit=read(OUT/f'qualification_{version}_audit.json')
        files=[(build/name,name) for name in ['configure.log','build.log','tests.log','CMakeCache.txt']]
        for folder in sorted(build.iterdir()):
            if folder.is_dir() and ('_cli_' in folder.name or folder.name.startswith('native_evidence_test_') or folder.name.startswith('round76_structural_')):
    ...
    raw=OUT/'diagnostic/local_raw'
    package('fixed_diagnostic',[(p,p.relative_to(raw).as_posix()) for p in sorted(raw.rglob('*')) if p.is_file()])
    manifest=dict(bundles=bundles,source_scope='All startup and fixed-diagnostic raw files, structural closure traces, all native qualification CLI/model/log trees and native Start fixture; full build/test logs, no executable binaries',
