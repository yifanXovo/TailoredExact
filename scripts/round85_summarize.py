"""Readable final tables derived only from closed audited R85 evidence."""
import json
import time
from pathlib import Path
from round85_analyze import ROOT, OUT, read, write, sha, compare
from round81_replication import endpoint
...
def main():
    ...
    result=dict(endpoints=rows,final_pairs=pairs,final_protection=protection,
        source_hashes={name:sha(OUT/name) for name in ['summary.json','audit.json','actual_calls.json','protection.json','replication.json']},
    ...
    lines=['# R85 audited result tables','',
