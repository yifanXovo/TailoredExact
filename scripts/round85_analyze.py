...
def main():
    ...
    assert summary['completed']==21 and all(r['audit']['passed'] for r in summary['records'])
    assert sha(ROOT/'scripts/round85_research.py')==frozen['driver_sha256']
    ...
    assert sha(ROOT/'results/unified_exact_round85/plan.md')==frozen['plan_sha256']
    ...
    assert len(frozen['launches'])==21 and len(summary['records'])==21
    ...
    assert len(all_checkpoints)==84 and len(endpoint_checks)==21
    consistency=cross_run_consistency(endpoint_checks+all_checkpoints)
    comparisons=[];protection=[]
    for identity in ['E7', 'S12', 'D3', 'C2', 'C6', 'C8', 'D6']:
        ...
        for t in checkpoint_times(representative['cap']):
            ...
            protection.append(dict(id=identity,seconds=t,fresh_K1_control_available=k is not None,
                ...
                scope='Fresh same-build exposed-development comparison only; all controls are current. No algorithm gate or historical-fastest veto'))
    assert len(comparisons)==84 and len(protection)==28
    ...
    audit=dict(all_checks_passed=True,completed=21,actual_optimize_calls=len(actual_calls),
