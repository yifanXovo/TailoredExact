"""R86 frozen four-role prospective ENS-C confirmation driver."""
import json
import os
...
ROLE_IDS=['F1','F2','F5','F6']
CAPS={'F1':120,'F2':300,'F5':3600,'F6':3600}
...
def main():
    ...
    assert subprocess.check_output(['git','branch','--show-current'],cwd=ROOT).decode().strip()=='codex/round86-ensc-varied-unadapted-confirmation'
    assert not subprocess.check_output(['git','status','--porcelain','--','src','include','tests','CMakeLists.txt','scripts/round86_research.py','scripts/round86_native_evidence.py','scripts/round86_analyze.py','scripts/round86_offline_validate.py','results/unified_exact_round86/preregistration.json','results/unified_exact_round86/generation.json','results/unified_exact_round86/plan.md','results/unified_exact_round86/protocol.json'],cwd=ROOT).strip(), 'Freeze source and driver first'
    ...
    assert read(STAGE/'base_publication.json')['head_sha']==read(STAGE/'preregistration.json')['base']
    assert read(STAGE/'generation.json')['generated_count']==4
    assert read(STAGE/'generation.json')['screened_or_rejected_count']==0
    assert read(STAGE/'protocol.json')['all_inputs_exposed']==False
    prior_stage=ROOT/'results/unified_exact_round85'
    ...
    offline=read(STAGE/'offline_validation_v2.json')
    assert offline['passed'] and offline['optimizer_calls']==0
    for name,digest in offline['script_sha256'].items():assert sha(ROOT/name)==digest,name
    ...
    assert len(roles)==12 and sum(CAPS[p['id']] for p,_,_ in roles)==22860
    ...
    identity = dict(source_commit=frozen['source_commit'],binary_sha256=sha(build/'ExactEBRP.exe'),
        ...
        preregistration_sha256=sha(STAGE/'preregistration.json'),generation_sha256=sha(STAGE/'generation.json'),
        offline_validation_sha256=sha(STAGE/'offline_validation_v2.json'),
        ...
        launches=launches,references=references,prior_bindings=read(STAGE/'protocol.json')['prior_bindings'],startup_summary_sha256=sha(ROUND83/'startup/completion_summary.json'),protocol_sha256=sha(STAGE/'protocol.json'),maximum_process_seconds=22860,formal_performance=True)
    ...
    write(STAGE/'full_preflight.json',dict(inherited_source=frozen['source_commit'],
        ...
        reference_builds=4,reference_optimize_calls=0,reference_wall_seconds=reference_wall,
    ...
    try:
        for launch in launches:
            ...
            try:
                ...
                if startup_exit:
                    ...
                    evidence.finalize_endpoint(ROOT,launch['panel'],launch['arm'],audited,observations,result,reason,references[launch['panel']['id']])
            ...
            write(OUT/'summary.json',dict(records=records,total_wall_seconds=sum(r['wall_seconds'] for r in records),
                ...
                completed=len(records),planned=12,formal_performance=True))
        ...
        lock.unlink()
        write(OUT/'driver_completion.json',dict(completed=len(records),planned=12,all_valid=len(records)==12 and all(r['audit']['passed'] for r in records),ended_unix=time.time()))
