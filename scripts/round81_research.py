"""Round81 frozen critical replication and common3600 D7 protection."""
import json
import os
...
from round78_qualify import ROOT, OUT as ROUND78, sha, write
...
STAGE=ROOT/'results/unified_exact_round81'
ROLE_IDS=['S12','C2','D4','D6','D7']
CAPS=dict(S12=120,C2=300,D4=300,D6=3600,D7=3600)
ARMS={i:['P-GRB','BDS-C'] if i=='D6' else ['P-GRB','BDS-C','K1-R'] for i in ROLE_IDS}
...
def prior_startup_folder(identity):
    if identity=='D7':return ROUND78/'startup/local_raw/D7/BDS-C'
    stage=79 if identity in ['S12','C2','D4'] else 80
    old=read(ROOT/f'results/unified_exact_round{stage}/campaign/identity.json')
    matches=[l for l in old['launches'] if l['panel']['id']==identity and l['arm']=='BDS-C']
    assert len(matches)==1
    return ROOT/matches[0]['destination']


def main():
    preflight_started=time.perf_counter()
    build = ROOT/'build/round78/v1'
    qualification = read(ROUND78/'qualification_v1.json')
    ...
    qa=read(ROUND78/'qualification_v1_audit.json')
    assert qa['passed'] and qa['actual_optimize_calls']==147
    assert sha(build/'ExactEBRP.exe')==qa['binary_sha256']
    assert read(ROUND78/'native_integration_audit.json')['passed']
    assert not (ROUND78/'qualification_active.lock').exists()
    assert not (ROUND78/'startup/active_run.lock').exists()
    assert subprocess.check_output(['git','branch','--show-current'],cwd=ROOT).decode().strip()=='codex/round81-bdsc-long-protection-replication'
    assert not subprocess.check_output(['git','status','--porcelain','--','src','include','tests','CMakeLists.txt','scripts/round81_research.py','results/unified_exact_round81/plan.md'],cwd=ROOT).strip(), 'Freeze measured source and driver before launch'
    startup=read(ROUND78/'startup/summary.json')
    assert read(ROUND78/'campaign/audit.json')['all_checks_passed']
    assert read(ROUND78/'campaign/mechanism_audit.json')['all_checks_passed']
    assert read(ROUND78/'prepublication_check.json')['all_current_source_binary_driver_hashes_match']
    assert read(ROOT/'results/unified_exact_round79/campaign/audit.json')['all_checks_passed']
    assert read(ROOT/'results/unified_exact_round79/campaign/mechanism_audit.json')['all_checks_passed']
    assert read(ROOT/'results/unified_exact_round80/campaign/audit.json')['all_checks_passed']
    assert read(ROOT/'results/unified_exact_round80/campaign/mechanism_audit.json')['all_checks_passed']
    assert read(STAGE/'prior_publication_verified.json')['head_sha']=='634cf03982e71849079699848ceecc3cd19ecb72'
    ...
    for i in ROLE_IDS:
        panel[i]=dict(panel[i],stage='development',role='Exposed frozen R81 replication/protection role')
    roles=[(panel[i],arm,False) for i in ROLE_IDS for arm in ARMS[i]]
    assert len(roles)==14 and sum(CAPS[p['id']] for p,_,_ in roles)==20160
    prior_bindings={}
    for i in ROLE_IDS:
        previous=prior_startup_folder(i)
        assert previous.exists()
        prior_bindings[i]=dict(folder=str(previous.relative_to(ROOT)),
            initial_sha256=sha(previous/'hga.csv.balanced/initial.json'),
            descent_sha256=sha(previous/'hga.csv.descent.csv'))
    ...
    for p in [panel[i] for i in ROLE_IDS]:
        ...
        old_refs=read(ROOT/'results/unified_exact_round71/fingerprints.json')
        assert references[p['id']]['canonical_sha256']==old_refs[p['id']]['canonical_sha256']
        assert references[p['id']]['fingerprint']==old_refs[p['id']]['fingerprint']
    launches = []
    for number, (p,arm,forced) in enumerate(roles,1):
        ...
        if arm == 'P-GRB':
            ...
            command += ['--algorithm-preset',{'BDS-C':'research-round78-vds-balanced-descent','K1-R':'research-round65-k1-h'}[arm],
    ...
    identity = dict(source_commit=frozen['source_commit'],binary_sha256=sha(build/'ExactEBRP.exe'),
        source=frozen['source'],qualification_sha256=sha(ROUND78/'qualification_v1.json'),
        ...
        launches=launches,references=references,prior_bindings=prior_bindings,maximum_process_seconds=20160,formal_performance=True)
    ...
    write(STAGE/'full_preflight.json',dict(inherited_source=frozen['source_commit'],
        inherited_binary_sha256=identity['binary_sha256'],inherited_ctest_count=60,
        ...
        reference_builds=5,reference_optimize_calls=0,reference_wall_seconds=reference_wall,
        ...
        for launch in launches:
            ...
            try:
                ...
                if launch['arm']=='BDS-C':
                    ...
                    folder=dest/'hga.csv.balanced'
                    initial=normalize(read(folder/'initial.json'))
                    prior_folder=prior_startup_folder(launch['panel']['id'])
                    bound=identity['prior_bindings'][launch['panel']['id']]
                    assert sha(prior_folder/'hga.csv.balanced/initial.json')==bound['initial_sha256']
                    assert sha(prior_folder/'hga.csv.descent.csv')==bound['descent_sha256']
                    if prior_folder.exists():
                        prior=normalize(read(prior_folder/'hga.csv.balanced/initial.json'))
                        assert route_hash(initial)==route_hash(prior)
                    audited['balanced']=replay_balanced(launch['panel'],folder,initial)
                    ...
                    if prior_folder.exists():
                        previous=prior_folder/'hga.csv.descent.csv'
            ...
            write(OUT/'summary.json',dict(records=records,total_wall_seconds=sum(r['wall_seconds'] for r in records),
                ...
                completed=len(records),planned=14,formal_performance=True))
            ...
            assert sum(r['audit'].get('native_calls_started',0) for r in records)<=200, 'Campaign resource ceiling; never an internal solver decision'
    finally:
        lock.unlink()
        write(OUT/'driver_completion.json',dict(completed=len(records),planned=14,all_valid=len(records)==14 and all(r['audit']['passed'] for r in records),ended_unix=time.time()))
