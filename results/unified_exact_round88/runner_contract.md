...
本轮现已冻结 A1 源码提交 `13e09e9b676e95d18bd635d37a5960903bad4113`，候选二进制 `build/research/round88-a1/ExactEBRP.exe` 的 SHA256 为 `23d4fd53602d46f599f3f58e33c3ec96c4eec01b972e22ee0d4815cc42a54693`；11/11 资格测试通过。已写新入口 `../../scripts/round88_a1_g3.py dry-run|prepare|run-smoke|run-rest` 与独立 G2 入口 `../../scripts/round88_startup_diagnostic.py dry-run|offline-replay|prepare|run`，精确场景、命令及顺序见两份 `preregistration_a1_*.json`。`dry-run`、`offline-replay` 和 `prepare` 均零 Optimize；正式 `run` 仍须冻结脚本审查、模型元数据确认、独占资源 lease。先完成 D6 的 ENS-C/A1 两次完整 startup-only 诊断，再按 E8、S12 两个小例三臂各一遍，独立审查后才允许其余六例。不能调用 R83/R86/R87 旧 batch 主入口。三臂均使用同一个 A1 build：P 原始 compact 加指纹与规范 SHA 核验，ENS 使用同构建默认关闭 A1 的冻结 preset，A1 在同 preset 上仅启用已资格化开关；R83 binary 只提供历史血缘信息。每臂按 `cap-6` 原生上限、`cap` 进程上限、`cap-2` 外部硬停及 3 秒 shutdown margin 执行，不切分内部组件。
