...
set(out "${OUTPUT_ROOT}/round73_cli_${suffix}")
if(EXISTS "${out}")
...
execute_process(COMMAND "${EXACT_EXE}"
  ...
  --method gcap-frontier --algorithm-preset research-round73-vds-joint-insertion
...
if(NOT exit_code EQUAL 0)
  message(FATAL_ERROR "Real JI CLI failed: ${exit_code}; artifacts=${out}")
endif()
file(READ "${out}/result.json" result)
foreach(field strict_certified_original_problem
  ...
  if(NOT value)
    message(FATAL_ERROR "JI CLI did not establish ${field}; artifacts=${out}")
  endif()
endforeach()
string(JSON mode GET "${result}" hga_stop_mode)
string(JSON method GET "${result}" primal_heuristic)
...
if(NOT mode STREQUAL "motif-exhaustion" OR NOT method STREQUAL "joint-insertion" OR NOT generations EQUAL 0
    OR upper LESS 0.208333233333 OR upper GREATER 0.208333433334
    OR lower LESS 0.208333233333 OR lower GREATER 0.208333433334)
  message(FATAL_ERROR "JI CLI mode/seed/original-objective mismatch; artifacts=${out}")
endif()
file(STRINGS "${out}/hga.csv.joint.csv" insertion_trace)
list(GET insertion_trace -1 final_insertion)
if(NOT final_insertion MATCHES "motif_exhausted")
  message(FATAL_ERROR "JI did not exhaust its finite motif neighborhood")
endif()
file(STRINGS "${out}/external/paper_optimize_ledger.csv" ledger)
list(LENGTH ledger rows)
math(EXPR calls "${rows} - 1")
if(calls LESS 1)
  message(FATAL_ERROR "JI CLI never entered the exact backend")
endif()
...
if(NOT starts)
  message(FATAL_ERROR "JI CLI did not exercise a native Start decision")
endif()
message(STATUS "Round73JointInsertionCliTests: real CLI certificate5/24; Optimize calls=${calls}; artifacts=${out}")
