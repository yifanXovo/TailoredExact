...
The repeated-LP service is not admitted to ENS-C. One follow-up represents the same entire B1/B2 family with absolute-value epigraphs and shared sparse station prefixes. Elimination proves equal projection; B2 needs no redundant combined arm. This is distinct from historical station-local MC4/VD-J, but its extra matrix size may still hurt runtime. The new implementation passes independent static review, 8/8 pure tests, two hand-computable Gurobi toys (B1 h=0, B2 h=1 with zero residuals), real Job cleanup and three zero-Optimize source audits. Root has signed six original-LP diagnostics, each with a 300-second whole-process cutoff; results are pending. This qualification costs 4.138076s at the helper boundary, separately from the earlier closure-tool qualification. See [sparse formulation proof](results/unified_exact_round88/ot_compact_epigraph_proposal.md).
