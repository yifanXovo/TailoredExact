#!/usr/bin/env python3
"""Run finalization/convergence diagnostics for paper-gf-tailored-bc.

The runner intentionally treats each hard fixed interval as a child process.
If the in-process CPLEX callback solve does not return control, the parent
writes an honest noncertified JSON from progress artifacts. Plain CPLEX rows
...
RESULTS = ROOT / "results" / "gf_tailored_bc_finalization_round"
RAW = RESULTS / "raw"
LOGS = RESULTS / "logs"
...
def wrapper_json(path: Path, *, row: str, instance: Path, gamma_l: float, gamma_u: float,
    ...
    data = {
        ...
        "certificate": "Dedicated worker/parent wrapper stopped the child before CPLEX returned a final best bound; no certificate claimed.",
        "certified_original_problem": False,
        ...
        "compact_bc_best_bound_fail_reason": "native_cplex_callback_did_not_return_before_parent_timeout",
        ...
        "plateau_reason": "no_valid_best_bound_emitted_before_parent_stop",
        "diagnostic_row": True,
        "paper_certificate_contamination": False,
        "plain_cplex_benchmark_used_as_certificate": False,
...
def run_interval(row: str, instance: Path, gl: float, gu: float, ub: float,
    ...
    if meta.get("timeout") or not out.exists():
        scope = "plain_fixed_interval_benchmark" if variant == "plain_fixed_interval" else "diagnostic_fixed_interval"
...
def write_report(rows: List[Dict[str, Any]], audits: List[Dict[str, Any]]) -> None:
    ...
    for r in hard:
        report.append(
            ...
            f"{r['classification']} | {r['progress_checkpoints']} | "
            f"{r['callback_abort_requests']} | {r['lower_bound']} | "
...
def main() -> int:
    ...
    write_csv(RESULTS / "hard_leaf_finalization_summary.csv", rows)
    write_csv(RESULTS / "best_bound_extraction_audit.csv", [
        {
            "row": r["row"],
    ...
    write_csv(RESULTS / "hard_leaf_longrun_classification.csv", [
        {
            **r,
            "failure_mode": (
                "C_no_valid_bound_emitted" if not b(r["best_bound_available"]) else
                "A_time_limited_but_improving" if b(r["compact_bc_bound_valid"]) and "timeout" in r["status"] else
                "solver_closed_or_reference"
            ),
        }
        for r in rows if "low_gini" in r["row"] or "hard" in r["row"]
    ])
