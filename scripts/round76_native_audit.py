...
def main():
    ...
    closure=list(build.glob('round76_closure_cli_*'));control=list(build.glob('round75_quantity_cli_*'))
    ...
    records=[]
    for arm,root in [('JDS-C',closure[0]),('JDS-X',control[0])]:
        folder=root/arm;result=read(folder/'result.json')
        if arm=='JDS-X':
            assert not (folder/'hga.csv.closure.csv').exists()
            assert 'Round76 physical closure' not in '\n'.join(result.get('notes',[]))
