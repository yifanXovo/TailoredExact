...
def check(folder,p,result,run,audit,route_hash):
    ...
    completion_event='decoded_descent_complete' if result.get('hga_stop_mode','').startswith('decoded-descent') else 'hga_generation_loop_complete'
    completed=next(r for r in phases if r['event']==completion_event)
