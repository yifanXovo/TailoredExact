...
def main():
    ...
    q=read(OUT/'qualification_v2_audit.json');assert q['passed']
    physical.ROOT=ROOT;mapping.run.ROOT=ROOT;mapping.audit.physical_module.ROOT=ROOT
    root=list((ROOT/'build/round75/v2').glob('round75_quantity_cli_*'));assert len(root)==1
    ...
    records=[]
    for arm in ['QDS-X','JDS-X']:
        folder=root[0]/arm;result=read(folder/'result.json')
