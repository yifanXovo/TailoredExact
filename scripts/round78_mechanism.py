...
from round78_analyze import campaign_summary
...
def main():
    ...
    previous=rows(ROOT/'results/unified_exact_round78/startup/local_raw/D7/BDS-C/hga.csv.descent.csv')
    assert list(map(logical,traces['BDS-C']))==list(map(logical,previous))
    output=dict(all_checks_passed=True,arms=arms,start_decisions=starts,controls_checked=controls,
        all25_paths_match_prior_startup_screen=True,actual_Start_decisions=len(starts),
