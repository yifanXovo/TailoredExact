...
def campaign_summary():
    ...
    if path.exists():
        ...
        assert correction['independent_checks_passed']
        assert correction['original_summary_sha256']==sha(OUT/'summary.json')
        index=correction['number']-1;record=summary['records'][index]
        assert record['arm']==correction['arm']=='K1-R' and index==3
        assert record['audit']==correction['original_audit'] and not record['audit']['passed']
        record['raw_supervisor_audit']=record['audit']
        record['audit']=correction['validated_audit']
...
def main():
    ...
    assert summary['completed']==4 and all(r['audit']['passed'] for r in summary['records'])
    ...
    assert sha(ROOT/'results/unified_exact_round74/plan.md')==frozen['plan_sha256']
    ...
    for launch,done in zip(frozen['launches'],summary['records']):
        ...
        for r in records:
            ...
            log=Path(c['native_log_path']);count=log.read_text(encoding='utf-8',errors='replace').count('Optimize a model')
    ...
    for t in [300,600,900,1200]:
        ...
        for a,b in [('P-GRB','DS-X'),('P-GRB','JDS-X'),('P-GRB','K1-R'),('DS-X','JDS-X'),('K1-R','JDS-X')]:
            comparisons.append(dict(seconds=t,reference=a,candidate=b,**compare(arms[a],arms[b])))
        p,k,j=arms['P-GRB'],arms['K1-R'],arms['JDS-X']
        fraction=None
    ...
    audit=dict(all_checks_passed=True,completed=4,actual_optimize_calls=len(actual_calls),
        ...
        supervisor_scope_correction_sha256=summary.get('supervisor_scope_correction_sha256'),
