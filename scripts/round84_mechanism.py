"""Offline actual startup/neighborhood/Start audit of the ENS-C complete-method screen."""
import csv
import json
import re
import time
...
from round84_analyze import campaign_summary
from round75_startup import normalize, route_hash

OUT=ROOT/'results/unified_exact_round84/campaign'
...
def main():
    ...
    for launch,done in zip(frozen['launches'],summary['records']):
        ...
        if arm not in ['BDS-C','ENS-C']:
            ...
            row['neutral_physical_audit']=done['audit']['neutral']
            row['outer_handoff']=done['audit']['outer_handoff']
            trace=rows(folder/'hga.csv.descent.csv');traces[(identity,arm)]=trace
            completed=[int(r['seed']) for r in trace if r['exhausted']=='1']
            count=len(completed)
            assert completed==list(range(1,count+1)) and count<=25
            mathematical_zero=row['neutral_physical_audit']['initial']['F']<=1e-7
            assert count==25 or mathematical_zero, 'Partial paths require a verified mathematical zero, not silence'
            for r in trace:
                assert r['interrupted']=='0' or mathematical_zero
                if r['exhausted']=='1':
                    assert r['decoded_checks']==r['neighbors'] and r['cross_route_checks']==r['cross_route_neighbors']
            row['descent']=dict(completed_seeds=count,all25_completed=count==25,mathematical_zero_stop=mathematical_zero,passes=len(trace),
        ...
        arms.append(row)
    assert len(traces)==3 and controls==6
    logical=lambda row:{k:v for k,v in row.items() if k!='elapsed_seconds'}
    previous=read(ROOT/'results/unified_exact_round83/startup/identity.json');prior_pairs=[]
    bindings=read(OUT.parent/'protocol.json')['prior_bindings']
    assert frozen['prior_bindings']==bindings
    for identity,arm in traces:
        candidates=[x for x in previous['commands'] if x['id']==identity and x['arm']==arm]
        assert candidates,'Every declared R84 ENS arm has a frozen R83 startup binding'
        assert len(candidates)==1
        prior=candidates[0];path=ROOT/prior['destination']/'hga.csv.descent.csv'
        bound=bindings[identity]
        assert Path(bound['folder'])==Path(prior['destination'])
        assert sha(path)==bound['descent_sha256']
        for name in ['initial','final']:
            assert sha(ROOT/prior['destination']/('hga.csv.exchange/'+name+'.json'))==bound[name+'_sha256']
            current_launch=next(l for l in frozen['launches'] if l['panel']['id']==identity and l['arm']==arm)
            old_witness=normalize(read(ROOT/prior['destination']/('hga.csv.exchange/'+name+'.json')))
            new_witness=normalize(read(ROOT/current_launch['destination']/('hga.csv.exchange/'+name+'.json')))
            assert route_hash(old_witness)==route_hash(new_witness), (identity,name,'current versus prior startup route')
        assert list(map(logical,traces[(identity,arm)]))==list(map(logical,rows(path)))
        prior_pairs.append(dict(id=identity,arm=arm,prior_trace=str(path.relative_to(ROOT)),prior_sha256=sha(path)))
    assert len(prior_pairs)==3
    output=dict(all_checks_passed=True,arms=arms,start_decisions=starts,controls_checked=controls,
        all25_paths_match_available_prior_startup_screen=True,prior_startup_pairs_checked=len(prior_pairs),prior_startup_pairs=prior_pairs,
        no_prior_routes_used_as_algorithm_input=True,actual_Start_decisions=len(starts),
