"""Bounded serial Round71 runs with uniform inherited measurement affinity."""
...
OUT=ROOT/'results/unified_exact_round71';BUILD=ROOT/'build/round71';RAW=OUT/'local_raw'
...
def freeze():
    ...
    old.update({p['id']:p for p in read(ROOT/'results/unified_exact_round70/revision2/protocol.json')['panel']})
    ...
    for identity in ids:
        ...
        p['role']='Round71 exposed development/protection; see plan.md' if p['stage']=='development' else 'Original tiny correctness only; micro-zero denotes zero handling'
        assert sha(ROOT/p['instance_path'])==p['input_sha256']
        selected.append(p)
    q=read(OUT/'qualification.json')
    assert q['final_ctest_passed_tests']==49 and q['final_binary_sha256']==sha(BUILD/'ExactEBRP.exe')
    ...
    caps=dict(E7=120,S12=120,N12=120,D3=300,C2=300,D4=300,D6=600,D7=1200,micro=30,**{'micro-zero':30})
    allowed={i:['P-GRB','DS','DS-X']+(['K1-R'] if i=='D7' else []) for i in ids}
    write(OUT/'protocol.json',dict(base='1cf8381206edd5925eca0b03185ac1cd7dc99d2a',panel=selected,
        plan_sha256=sha(OUT/'plan.md'),mathematics_sha256=sha(OUT/'mathematics.md'),
        maximum_performance=25,maximum_micro=6,maximum_reference_exports=10,worst_case_seconds=10560,
        caps=caps,allowed_arms=allowed,arms=['P-GRB','K1-R','DS','DS-X'],
        ...
        candidate='24 finite decoded descents including uniform cross-route tail relocation, before unchanged VD-S proof; all internal resource policies off',
        confirmation_opened=False,validation_scope='All roles exposed development/protection; New qualified binary requires fresh matched controls under the same core condition'))
...
def execute(cmd,dest,identity,cap,kind='performance',solver_calls=1):
    ...
    assert not (ROOT/'results/unified_exact_round70/revision2/active_run.lock').exists()
...
def refbuild(ids):
    ...
    previous.update(read(ROOT/'results/unified_exact_round70/revision2/fingerprints.json'))
    for identity in ids:
    ...
    for identity in ids:
        ...
        for arm in arms:
            ...
            if arm=='P-GRB':
                ...
                preset={'K1-R':'research-round65-k1-h','DS':'research-round70-vds-descent','DS-X':'research-round71-vds-interroute-descent'}[arm]
            ...
            if arm!='P-GRB':
                valid_tree=result['external_gini_tree_root_coverage_valid'] and result['external_gini_tree_parent_child_coverage_valid']
                if not valid_tree:
                    assert result['status']=='paper_hga_global_deadline',result['status']
                    ...
                    assert not (dest/'external').exists(),'Unexpected partial proof requires full audit'
                    # Full independent physical/nonnegative-bound audit remains
                    # mandatory at the queue boundary; this is not a certificate.

            if arm in ['DS','DS-X']:
                expected='decoded-descent-interroute' if arm=='DS-X' else 'decoded-descent'
                assert result['hga_stop_mode']==expected and result['hga_total_generations']==0
                assert result['decoded_descent_cross_route_enabled']==(arm=='DS-X')

if __name__=='__main__':
    parser=argparse.ArgumentParser();parser.add_argument('action',choices=['freeze','refbuild','run'])
    parser.add_argument('--ids',nargs='+',default=[]);parser.add_argument('--arms',nargs='+',default=['P-GRB','DS','DS-X'])
