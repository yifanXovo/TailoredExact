...
def main():
    ...
    assert summary['completed']==13 and all(r['audit']['passed'] for r in summary['records'])
    assert sha(ROOT/'scripts/round83_research.py')==frozen['driver_sha256']
    ...
    assert sha(ROOT/'results/unified_exact_round83/full_screen_plan.md')==frozen['plan_sha256']
    for name,digest in frozen['source'].items():assert sha(ROOT/name)==digest,name
    assert sha(OUT.parent/'startup/completion_summary.json')==frozen['startup_summary_sha256']
    ...
    audit=dict(all_checks_passed=True,completed=13,actual_optimize_calls=len(actual_calls),
