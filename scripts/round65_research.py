...
def execute(p,arm,cap,stage,kind='performance',credit=30):
    ...
    if arm=='P-GRB':
        ...
        cmd+=['--round65-budget',str(mode in ['bounded','proof','sparse','bounded-joint']).lower(),
              ...
              '--round65-seed-credit',credit,'--round65-hga-zero-stop',str(mode=='reliable').lower(),
...
def main():
    p=argparse.ArgumentParser();p.add_argument('action',choices=['freeze','build-freeze','run','micro'])
