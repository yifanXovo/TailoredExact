#!/usr/bin/env python3
"""Create clean Round 29 release builds and run every repository test."""
...
def execute(name: str, command: list[str],
    ...
    write_json(OUT / "build_and_test_record.partial.json", {
        "schema": "round29-build-tests-v1",
        "records": records,
    })
    print(
        f"{name}: rc={completed.returncode} "
        f"wall={record['wall_seconds']:.3f}",
        flush=True)
...
def main() -> int:
    if CPLEX_BUILD.exists() or GUROBI_BUILD.exists():
        raise SystemExit(
            "clean-build destinations already exist; retain them and audit "
            "rather than deleting or silently reconfiguring")
    OUT.mkdir(parents=True, exist_ok=True)
    records: list[dict[str, Any]] = []
    try:
        execute("stage0_configure_cplex", configure_command(
            CPLEX_BUILD, False), records)
        execute("stage0_build_cplex", [
            str(CMAKE), "--build", str(CPLEX_BUILD), "-j", "8"], records)
        execute("stage0_ctest_cplex", [
            str(CTEST), "--test-dir", str(CPLEX_BUILD),
            "--output-on-failure"], records)
        execute("stage0_configure_gurobi", configure_command(
            GUROBI_BUILD, True), records)
        execute("stage0_build_gurobi", [
            str(CMAKE), "--build", str(GUROBI_BUILD), "-j", "8"], records)
    ...
    result = {
        ...
        "cpp_test_count_per_build": 12,
        "python_test_script_count": len(list(
            (ROOT / "tests").glob("*.py"))),
        "records": records,
