"""Offline actual startup/neighborhood/Start audit of the frozen five-role replication/protection runs."""
import csv
import json
import re
import time
...
from round81_analyze import campaign_summary
...
def main():
    ...
    for identity in ['S12','C2','D4','D6','D7']:
        binding=frozen['prior_bindings'][identity]
        prior_folder=ROOT/binding['folder']
        assert sha(prior_folder/'hga.csv.descent.csv')==binding['descent_sha256']
        assert sha(prior_folder/'hga.csv.balanced/initial.json')==binding['initial_sha256']
        previous=rows(prior_folder/'hga.csv.descent.csv')
        assert list(map(logical,traces[identity]))==list(map(logical,previous))
        prior_checks+=1
    assert len(traces)==5 and controls==9
