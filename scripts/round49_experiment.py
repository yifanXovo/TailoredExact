#!/usr/bin/env python3
"""Run and seal one frozen Round 49 K1-AM-RC diagnostic row."""
...
RULE = "d-rcd"
ARM = "K1-AM-RC-A"
REQUIRED_LIVE_LEDGERS = (
    "rc_decision_ledger.csv", "rc_variable_domain_ledger.csv",
    "lp_primal_dual_ledger.csv", "native_target_ledger.csv",
    "interval_tree_events.csv", "interval_coverage_ledger.csv",
    "global_bound_trace.csv", "model_size_ledger.csv",
    "certificate_ledger.csv",
)
REQUIRED = (
    "command.json", "process_manifest.json", "process_phases.csv",
    "progress.csv", "result.json", *REQUIRED_LIVE_LEDGERS,
    "primitive_integer_variable_registry.csv", "artifact_manifest.csv",
...
def seal(run_dir: Path, record: dict[str, Any]) -> None:
    ...
    copy_required(common.OUT / "primitive_integer_variable_registry.csv",
                  run_dir / "primitive_integer_variable_registry.csv")
    result = normalized_json(run_dir / "result.json")
    if result.get("round49_k1_am_rc") != RULE:
        raise RuntimeError("sealed row is not frozen K1-AM-RC D-RCD")
    if result.get("round49_rc_extra_lp_count") != 0 or \
            result.get("round49_rc_extra_mip_count") != 0:
        raise RuntimeError("K1-AM-RC reported an extra score solve")
    if result.get("round48_k1_amf") != "off":
        raise RuntimeError("AMF-v1 must remain off")
    ...
    common.write_json(run_dir / "completion_marker.json", {
        "schema": "round49-sealed-completion-marker-v1",
        "complete": True,
        ...
        "decision_count": result.get("round49_rc_decision_count", 0),
        "reduced_cost_valid_decision_count": result.get(
            "round49_rc_valid_decision_count", 0),
        "rescue_count": result.get("round49_rc_rescue_count", 0),
        "invalid_profile_fallback_count": result.get(
            "round49_rc_invalid_profile_fallback_count", 0),
        "extra_lp_count": result.get("round49_rc_extra_lp_count", 0),
        "extra_mip_count": result.get("round49_rc_extra_mip_count", 0),
    })


def run_one(args: argparse.Namespace, item: dict[str, Any],
            executable: Path) -> None:
    run_id = f"{args.stage}__{item['instance']}__{ARM}"
    ...
    command = common.candidate_command(
        item, run_dir, args.process_cap, executable, RULE)
    identity = common.identity(RULE)
    record = {
        "schema": "round49-run-v1",
        "round_id": 49,
        "stage": args.stage,
        "run_id": run_id,
    ...
    common.write_json(run_dir / "process_manifest.json", {
        "schema": "round49-process-manifest-v1",
        "run_id": run_id,
        "algorithm": ARM,
        "rule": RULE,
        "K0": 1,
        ...
        "primitive_registry_version": identity["primitive_registry_version"],
    ...
    if return_code or watchdog or not record["completed"]:
        raise RuntimeError(f"Round 49 row failed: {run_id}")
    seal(run_dir, record)
    result = normalized_json(run_dir / "result.json")
    print(json.dumps({
        "run_id": run_id,
        ...
        "decisions": result.get("round49_rc_decision_count", 0),
        "valid_profiles": result.get("round49_rc_valid_decision_count", 0),
        "rescues": result.get("round49_rc_rescue_count", 0),
        "fallbacks": result.get(
            "round49_rc_invalid_profile_fallback_count", 0),
...
def main() -> int:
    ...
    if not executable.is_file():
        raise SystemExit(f"Round 49 executable missing: {executable}")
    ...
    if unknown:
        raise SystemExit(
            f"instances are not in the Stage 3 panel: {sorted(unknown)}")
