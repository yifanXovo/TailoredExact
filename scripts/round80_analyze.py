...
def main():
    ...
    assert sha(ROOT/'scripts/round80_research.py')==frozen['driver_sha256']
    ...
    assert sha(ROOT/'results/unified_exact_round80/plan.md')==frozen['plan_sha256']
    ...
    for launch,done in zip(frozen['launches'],summary['records']):
        ...
        for t in ([30,60,120] if launch['cap']==120 else [300,600,1200,1800,2400,3600]):
    ...
    audit=dict(all_checks_passed=True,completed=6,actual_optimize_calls=len(actual_calls),
