#!/usr/bin/env python3
"""Checksum-resumable serial runner for frozen Round 35 SIMPLE rows.
...
LOCK = common.OUT / ".round35_runner.lock"
...
def validate_frozen(item: dict[str, Any], manifest: dict[str, Any]) -> None:
    for key in (
        "protocol", "source_of_truth", "instance_manifest", "matrix_1800",
        "matrix_3600", "repeat_freeze", "historical_compatibility",
        "fingerprints", "frozen_c6_equivalence", "official_matrix",
        "command_freeze", "stage0_build_and_tests",
    ):
    ...
    if (not common.EXE.is_file()
            or common.sha256(common.EXE) != manifest["gurobi_executable_sha256"]):
...
def run_one(row: dict[str, str], items: dict[str, dict[str, Any]],
    ...
    if directory.exists():
        ...
        common.write_json(directory / "invalidation_record.json", {
            "round_id": 35, "run_id": row["run_id"], "reason": reason,
    ...
    frozen_command = common.load_json(common.COMMAND_FREEZE)["commands"][
        row["run_id"]]["command"]
    ...
    if not finalized:
        common.write_json(directory / "run_state.json", {
            **record, "runner_state": "incomplete_preserved_requires_invalidation"})
        raise RuntimeError(
    ...
    print(
        f"DONE {row['serial_order']}/52 {row['run_id']} "
...
def acquire_lock() -> None:
    lock = common.load_json(LOCK) if LOCK.is_file() else {}
    if lock and pid_running(int(lock.get("pid", 0))):
        raise RuntimeError("another Round 35 runner is active")
...
def main() -> int:
    ...
    if not START_RECORD.is_file():
        common.write_json(START_RECORD, {
            "schema": "round35-official-start-v1", "round_id": 35,
    ...
    try:
        for row in rows:
            ...
            common.write_csv(
                SUMMARY,
                sorted(keyed.values(), key=lambda item: int(item["serial_order"])))
    finally:
        LOCK.unlink(missing_ok=True)
    print(json.dumps({
