...
def repeatability(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for instance in frozen.STAGE4_INSTANCES:
        ...
        for row, signature in zip(group, signatures):
            ...
            scalar_identity = bool(baseline) and all(row[field] == baseline[field]
                for field in ("hga_generations", "hga_no_improve_generations",
                              "hga_improvement_count", "hga_decoder_calls",
                              "hga_final_fitness", "hga_verified_objective",
                              "split_count", "lp_optimize_count",
                              "terminal_mip_optimize_count", "optimize_count",
                              "certificate_class"))
            lb_close = bool(baseline) and finite(row["valid_final_lb"]) and finite(
                baseline["valid_final_lb"]) and abs(number(row["valid_final_lb"]) -
                number(baseline["valid_final_lb"])) <= 1e-9
            output.append({
                ...
                "structural_identity_with_stage2": structural_identity,
                "scalar_identity_with_stage2": scalar_identity,
                "final_lb_within_1e-9": lb_close,
                "deterministic_identity": structural_identity and scalar_identity and lb_close,
            })
    return output
...
def dynamic_audits(rows: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], ...]:
    ...
    for row in rows:
        ...
        if external:
            ...
            ledger_ok = bool(ledger) and all(truth(item.get("exact_coverage")) and
            ...
            inherited = csv_rows(row["_run_dir"] / "external/bound_inheritance_ledger.csv")
            inherited_ok = bool(inherited) and all(truth(item.get("inheritance_valid")) and
        ...
        lifecycle.append({key: row[key] for key in (
            "stage", "instance", "arm", "status", "authoritative", "work", "lp_work",
            "terminal_mip_work", "lp_optimize_count", "terminal_mip_leaf_count",
...
def totals(rows: list[dict[str, Any]]) -> dict[str, dict[str, float]]:
    fields = ("work", "lp_work", "terminal_mip_work", "lp_optimize_count",
              ...
              "barrier_iterations", "native_nodes")
...
def main() -> int:
    ...
    time_limited = sum("time_limit" in str(row["status"]).lower() or
                       number(row["global_deadline_interruptions"]) > 0 for row in rows)
    strict_count = sum(truth(row["strict_certificate"]) for row in rows)
    pair_gap_wins = sum(number(item["right_minus_left_gap"]) < -1e-9 for item in p_pairs)
    pair_auc_wins = sum(number(item["right_minus_left_auc"]) > 1e-9 for item in p_pairs)
    ...
    for family in sorted({item["family"] for item in p_pairs}):
        ...
        family_summary.append({
            ...
            "C3_final_gap_wins": sum(number(item["right_minus_left_gap"]) < -1e-9
                                      for item in selected),
            "C3_AUC_wins": sum(number(item["right_minus_left_auc"]) > 1e-9
                               for item in selected),
            "median_C3_minus_P_gap": median(
                (number(item["right_minus_left_gap"]) for item in selected)),
            "median_C3_minus_P_AUC": median(
                (number(item["right_minus_left_auc"]) for item in selected)),
        })
    audit = {
        ...
        "future_incremental_reoptimization_justified": classification ==
            "algorithmically_equivalent_but_restart_costly",
    ...
    family_lines = "\n".join(
        f"- {item['family']}: {item['instances']} pairs; C3 final-gap wins "
...
C3 won {pair_gap_wins}/{len(p_pairs)} final common-UB gaps and
{pair_auc_wins}/{len(p_pairs)} bound-progress AUC comparisons.
...
The repeatability matrix compares the Stage 2 baseline plus two additional C3
runs for each of V12_M2, moderate3302, and moderate6301 using semantic split,
leaf, row-signature, and optimize-sequence hashes. Fresh-model startup evidence
shows a median build/read share of {fmt(median_startup_share)} of exact-phase
wall time. Future incremental reoptimization is
{str(audit['future_incremental_reoptimization_justified']).lower()} based on
the measured classification; it was not implemented in Round 28.
