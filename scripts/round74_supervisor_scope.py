...
def main():
    ...
    assert len(mip)==1
    assert mip[0]['lower_g']-1e-7<=physical_event['G']<=mip[0]['upper_g']+1e-7
    mapping.run.ROOT=ROOT;mapping.audit.physical_module.ROOT=ROOT
    model_check=mapping.check_model(p,read(folder/'external/initial_witness.json'),Path(mip[0]['model_path']))
    assert model_check['compatible_interval'] and model_check['compatible_cutoff']
    assert model_check['all_bounds_types_rows_valid']
    ...
    correction=dict(independent_checks_passed=True,number=4,arm='K1-R',
        ...
        reason='The original contract requires a physical same-run UB and complete-domain LB, not a native MIPSOL witness. The retained HGA point is independently feasible in the active MIP model, even though native search returns with no incumbent. The outer witness was not a native solver observation.',
        ...
        offline_initial_witness_model_check=model_check,mapping_sha256=sha(mapping.__file__),
