...
from round83_analyze import campaign_summary

OUT=ROOT/'results/unified_exact_round83/campaign'
...
def main():
    ...
    for launch,done in zip(frozen['launches'],summary['records']):
        ...
        arms.append(row)
    assert len(traces)==5 and controls==8
    logical=lambda row:{k:v for k,v in row.items() if k!='elapsed_seconds'}
    previous=read(OUT.parent/'startup/identity.json');prior_pairs=[]
    ...
    assert len(prior_pairs)==4
    assert list(map(logical,traces[('U6','BDS-C')]))==list(map(logical,traces[('U6','ENS-C')]))
