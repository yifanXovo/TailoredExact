"""Nine new run trees plus original references; inherited qualification stays bound."""
import gzip
import hashlib
import io
import json
...
def main():
    ...
    manifest=dict(bundles=bundles,
        source_scope='All nine original complete-run trees and three original compact exports. Exact source/qualification/startup inherited from byte-bound R83 bundles; no executable, license or new qualification.',
    ...
    mechanism=read(campaign/'mechanism_audit.json');replication=read(campaign/'replication_audit.json')
    ...
    resource=dict(new_qualification_calls=0,new_release_builds=0,new_ctests=0,new_native_fixture_calls=0,
        ...
        mechanism_audit_seconds=mechanism['wall_seconds'],replication_audit_seconds=replication['wall_seconds'],
