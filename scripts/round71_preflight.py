"""Record configure/build/test attempts before opening Round71 experiments."""
import argparse, datetime, hashlib, json, os, subprocess, time
from pathlib import Path

ROOT=Path(__file__).resolve().parents[1]
OUT=ROOT/'results/unified_exact_round71'
BUILD=ROOT/'build/round71'
...
def main(action):
    ...
    assert subprocess.check_output(['git','branch','--show-current'],cwd=ROOT).decode().strip()=='codex/round71-interroute-descent'
