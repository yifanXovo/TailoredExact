# Run the new preset through native proof. The inherited R75 CLI test supplies
# the same-binary JDS-X control without an additional duplicate Optimize run.
string(RANDOM LENGTH 16 ALPHABET 0123456789abcdef suffix)
set(root "${OUTPUT_ROOT}/round76_closure_cli_${suffix}")
if(EXISTS "${root}")
...
foreach(arm JDS-C)
  ...
  set(preset research-round76-vds-physical-closure)
  ...
  file(READ "${out}/hga.csv.closure.csv" closure)
  if(NOT closure MATCHES "initial_verified" OR NOT closure MATCHES "joint_neighborhood_exhausted" OR
      closure MATCHES "verification_rejected" OR NOT result MATCHES "Round76 physical closure")
    message(FATAL_ERROR "Closure preset did not run a verified physical descent; artifacts=${out}")
  endif()
  foreach(suffix initial final)
    if(NOT EXISTS "${out}/hga.csv.closure.csv.${suffix}.json")
      message(FATAL_ERROR "Closure physical snapshot missing: ${suffix}")
    endif()
  endforeach()
  ...
  message(STATUS "Round76PhysicalClosureCliTests ${arm}: certificate5/24; Optimize calls=${calls}; artifacts=${out}")
endforeach()
