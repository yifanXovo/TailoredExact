...
"""
    ...
    long_run_timestamp = """# Long-run callback timestamp root cause
...
"""
    write_text(
        OUT / "long_run_callback_timestamp_root_cause.md",
        long_run_timestamp)
...
The first long-run matrix then exposed a second telemetry-only issue:
Gurobi callback wall-clock runtime moved backward after a host clock
correction. Callback event placement now uses local monotonic elapsed time.
The original row and incomplete derived report are retained in invalidated
evidence, and every official row is uniformly rebuilt, rehashed, and rerun.
