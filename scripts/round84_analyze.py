...
def main():
    ...
    assert summary['completed']==9 and all(r['audit']['passed'] for r in summary['records'])
    assert sha(ROOT/'scripts/round84_research.py')==frozen['driver_sha256']
    ...
    assert sha(ROOT/'results/unified_exact_round84/plan.md')==frozen['plan_sha256']
    ...
    assert len(frozen['launches'])==9 and len(summary['records'])==9
    ...
    assert len(comparisons)==54 and len(protection)==18
    ...
    audit=dict(all_checks_passed=True,completed=9,actual_optimize_calls=len(actual_calls),
