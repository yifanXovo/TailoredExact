#!/usr/bin/env python3
"""Run and seal frozen Round 47 AM/AMC rows."""
...
REQUIRED = (
    ...
    "adaptive_mass_decision_ledger.csv", "contraction_ledger.csv",
    "native_target_ledger.csv", "native_optimize_ledger.csv",
    "certificate_ledger.csv", "model_size_ledger.csv",
...
def add_run_id(source: Path, target: Path, run_id: str) -> None:
    if not source.is_file():
        raise RuntimeError(f"required ledger missing: {source}")
...
def seal(run_dir: Path, record: dict[str, Any]) -> None:
    ...
    for name in ("adaptive_mass_decision_ledger.csv", "contraction_ledger.csv"):
        add_run_id(external / name, run_dir / name, record["run_id"])
    ...
    common.write_csv(run_dir / "model_size_ledger.csv", [{
        ...
        "model_reuse_count":
            result.get("external_gini_tree_model_reuse_count", 0),
        "basis_reuse_count":
            result.get("external_gini_tree_basis_reuse_count", 0),
        "peak_memory_gb":
        ...
        "contractions":
            result.get("round47_single_child_contraction_count", 0),
    }])
    ...
    common.write_json(run_dir / "completion_marker.json", {
        "schema": "round47-completion-marker-v1", "complete": True,
...
def run_one(args: argparse.Namespace, item: dict[str, Any],
    ...
    identity = common.identity(args.arm)
    ...
    command = common.candidate_command(
        item, run_dir, args.process_cap, args.arm, executable)
    definition = common.ARM_DEFINITIONS[args.arm]
    record = {
        "schema": "round47-run-v1", "round_id": 47,
        "stage": args.stage, "run_id": run_id,
        "instance_id": item["instance"], "instance_path": item["path"],
        "instance_sha256": item["sha256"], "arm": args.arm,
        "K0": definition["K0"], "mode": definition["mode"],
        "tau": common.TAU, "tau_explicit": True,
    ...
    common.write_json(run_dir / "process_manifest.json", {
        "schema": "round47-process-manifest-v1", "run_id": run_id,
        "arm": args.arm, "K0": definition["K0"],
        "mode": definition["mode"], "tau": common.TAU,
    ...
    if return_code or watchdog or not record["completed"]:
        raise RuntimeError(f"Round 47 row failed: {run_id}")
    seal(run_dir, record)
    result = normalized_json(run_dir / "result.json")
    print(json.dumps({
...
def main() -> int:
    ...
    if not executable.is_file():
        raise SystemExit(f"Round 47 executable missing: {executable}")
    if (args.stage.lower().startswith(("stage3", "stage4", "stage5")) and
            "dev-gurobi-release" in executable.as_posix().lower()):
        raise SystemExit(
            "official Stage 3/4/5 rows cannot use development executable")
