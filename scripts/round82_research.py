"""Round82 prospectively fixed six-role unadapted confirmation."""
import json
import os
...
ROLE_IDS=['U1','U2','U3','U4','U5','U6']
CAPS=dict(U1=120,U2=120,U3=300,U4=3600,U5=3600,U6=3600)
ARMS={i:['P-GRB','BDS-C','K1-R'] for i in ROLE_IDS}
...
def main():
    ...
    assert subprocess.check_output(['git','branch','--show-current'],cwd=ROOT).decode().strip()=='codex/round82-bdsc-unadapted-confirmation'
    assert not subprocess.check_output(['git','status','--porcelain','--','src','include','tests','CMakeLists.txt','scripts/round82_research.py','results/unified_exact_round82/plan.md','results/unified_exact_round82/protocol.json','results/unified_exact_round82/preregistration.json','scripts/round82_generate.py'],cwd=ROOT).strip(), 'Freeze measured source and driver before launch'
    ...
    assert read(STAGE/'prior_publication_verified.json')['head_sha']=='298e97f5d163a54eb9d899c664cafd9f07c8647c'
    assert read(ROOT/'results/unified_exact_round81/campaign/audit.json')['all_checks_passed']
    assert read(ROOT/'results/unified_exact_round81/campaign/mechanism_audit.json')['all_checks_passed']
    generation=read(STAGE/'generation.json')
    assert generation['generated_count']==6 and generation['screened_or_rejected_count']==0
    assert sha(STAGE/'plan.md')==generation['plan_sha256']
    assert sha(STAGE/'preregistration.json')==generation['preregistration_sha256']
    for path,digest in generation['generators'].items():assert sha(ROOT/path)==digest
    ...
    assert len(roles)==18 and sum(CAPS[p['id']] for p,_,_ in roles)==34020
    ...
    identity = dict(source_commit=frozen['source_commit'],binary_sha256=sha(build/'ExactEBRP.exe'),
        ...
        launches=launches,references=references,prior_bindings={},generation_sha256=sha(STAGE/'generation.json'),protocol_sha256=sha(STAGE/'protocol.json'),maximum_process_seconds=34020,formal_performance=True)
    ...
    write(STAGE/'full_preflight.json',dict(inherited_source=frozen['source_commit'],
        ...
        reference_builds=6,reference_optimize_calls=0,reference_wall_seconds=reference_wall,
        ...
        for launch in launches:
            ...
            try:
                ...
                if launch['arm']=='BDS-C':
                    ...
                    audited['prior_startup_binding']='none: genuinely new inputs; no archived path consulted'
            ...
            write(OUT/'summary.json',dict(records=records,total_wall_seconds=sum(r['wall_seconds'] for r in records),
                ...
                completed=len(records),planned=18,formal_performance=True))
            ...
            assert sum(r['audit'].get('native_calls_started',0) for r in records)<=300, 'Campaign resource ceiling; never an internal solver decision'
    finally:
        lock.unlink()
        write(OUT/'driver_completion.json',dict(completed=len(records),planned=18,all_valid=len(records)==18 and all(r['audit']['passed'] for r in records),ended_unix=time.time()))
