"""Round85 frozen remaining ENS-C protection and one D6 repetition."""
import json
import os
...
STAGE=ROOT/'results/unified_exact_round85'
ROLE_IDS=['E7', 'S12', 'D3', 'C2', 'C6', 'C8', 'D6']
CAPS={'E7': 120, 'S12': 120, 'D3': 300, 'C2': 300, 'C6': 600, 'C8': 600, 'D6': 3600}
...
def main():
    ...
    assert subprocess.check_output(['git','branch','--show-current'],cwd=ROOT).decode().strip()=='codex/round85-ensc-remaining-protection-replication'
    assert not subprocess.check_output(['git','status','--porcelain','--','src','include','tests','CMakeLists.txt','scripts/round85_research.py','results/unified_exact_round85/plan.md','results/unified_exact_round85/protocol.json'],cwd=ROOT).strip(), 'Freeze source and driver first'
    ...
    assert read(STAGE/'base_publication.json')['head_sha']=='ed0cd977ac2ea1cf7a7bf1236a15dd845a3ed948'
    ...
    assert len(roles)==21 and sum(CAPS[p['id']] for p,_,_ in roles)==16920
    ...
    identity = dict(source_commit=frozen['source_commit'],binary_sha256=sha(build/'ExactEBRP.exe'),
        ...
        launches=launches,references=references,prior_bindings=read(STAGE/'protocol.json')['prior_bindings'],startup_summary_sha256=sha(ROUND83/'startup/completion_summary.json'),protocol_sha256=sha(STAGE/'protocol.json'),maximum_process_seconds=16920,formal_performance=True)
    ...
    write(STAGE/'full_preflight.json',dict(inherited_source=frozen['source_commit'],
        ...
        reference_builds=7,reference_optimize_calls=0,reference_wall_seconds=reference_wall,
    ...
    try:
        for launch in launches:
            ...
            write(OUT/'summary.json',dict(records=records,total_wall_seconds=sum(r['wall_seconds'] for r in records),
                ...
                completed=len(records),planned=21,formal_performance=True))
        ...
        lock.unlink()
        write(OUT/'driver_completion.json',dict(completed=len(records),planned=21,all_valid=len(records)==21 and all(r['audit']['passed'] for r in records),ended_unix=time.time()))
