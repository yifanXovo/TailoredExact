...
from round83_qualify import ROOT,OUT,read,write,sha

def main():
    version=sys.argv[1];started=time.perf_counter()
    destination=OUT/f'qualification_{version}_audit.json'
    assert not destination.exists()
    build=ROOT/'build/round83'/version;q=read(OUT/f'qualification_{version}.json')
    ...
    passed='100% tests passed, 0 tests failed out of 62' in text
    ...
    failed_cli=[]
    for path in build.glob('round83_exchange_cli_*/*/result.json'):
        result=read(path)
    ...
    oracle=re.search(r'Round83 exchange structural:.*',text)
    oracle=oracle[0] if oracle else None
    if passed:assert oracle
    report=dict(version=version,passed=passed,tests_total=62,tests_passed=len(re.findall(r'\d+/62 Test .*Passed',text)),
