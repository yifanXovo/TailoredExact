"""Offline actual startup/neighborhood/Start audit of the frozen D7 runs."""
import csv
import json
import re
import time
...
from round74_analyze import campaign_summary
...
def main():
    ...
    for launch,done in zip(frozen['launches'],summary['records']):
        arm=launch['arm'];folder=ROOT/launch['destination'];p=launch['panel']
        ...
        row=dict(arm=arm,wall_seconds=done['wall_seconds'],endpoint=done['audit']['endpoint'],
        ...
        if arm not in ['DS-X','JDS-X']:
            ...
            for c in mip_calls:
                ...
                starts.append(dict(arm=arm,call=c['call'],eligible=eligible,native_log_accepted=accepted,
                    metadata=str(meta.relative_to(ROOT)),metadata_sha256=sha(meta),
                    witness_sha256=sha(witness_path),**data,**actual))
            trace=rows(folder/'hga.csv.descent.csv');traces[arm]=trace
            count=24 if arm=='DS-X' else 25
    ...
    assert list(map(logical,traces['DS-X']))==[logical(r) for r in traces['JDS-X'] if int(r['seed'])<=24]
    output=dict(all_checks_passed=True,arms=arms,start_decisions=starts,controls_checked=controls,
        first24_logical_paths_equal=True,actual_Start_decisions=len(starts),
