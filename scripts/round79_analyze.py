...
OUT=ROOT/'results/unified_exact_round79/campaign'
...
def main():
    ...
    assert summary['completed']==18 and all(r['audit']['passed'] for r in summary['records'])
    assert sha(ROOT/'scripts/round79_research.py')==frozen['driver_sha256']
    ...
    assert sha(ROOT/'results/unified_exact_round79/plan.md')==frozen['plan_sha256']
    ...
    for launch,done in zip(frozen['launches'],summary['records']):
        ...
        for t in ([30,60,120] if launch['cap']==120 else [60,120,180,300]):
    ...
    for identity in ['E7','S12','N12','D3','C2','D4']:
        ...
        for t in times:
            ...
            if all(v['available'] for v in [p,k,j]):
            ...
            protection.append(dict(id=identity,seconds=t,retained_K1_P_relative_advantage=fraction,metric=metric,
                K1_certificate_advantage=k['certificate'] and not p['certificate'],
                candidate_retains_that_certificate=j['certificate'] if k['certificate'] and not p['certificate'] else None,
                loss_of_most_advantage_at_this_checkpoint=fraction is not None and fraction<.5,
                scope='Descriptive exposed-role comparison; no algorithm gate or historical-fastest veto'))
    ...
    audit=dict(all_checks_passed=True,completed=18,actual_optimize_calls=len(actual_calls),
