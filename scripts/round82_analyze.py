...
def main():
    ...
    assert sha(ROOT/'scripts/round82_research.py')==frozen['driver_sha256']
    ...
    assert sha(ROOT/'results/unified_exact_round82/plan.md')==frozen['plan_sha256']
    for name,digest in frozen['source'].items():assert sha(ROOT/name)==digest,name
    assert sha(OUT.parent/'generation.json')==frozen['generation_sha256']
    ...
    for identity in ['U1','U2','U3','U4','U5','U6']:
        ...
        for t in checkpoint_times(representative['cap']):
            ...
            protection.append(dict(id=identity,seconds=t,fresh_K1_control_available=k is not None,
                ...
                scope='Fresh unadapted-role comparison only; all controls are current. No algorithm gate or historical-fastest veto'))
