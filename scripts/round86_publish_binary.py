...
OUT=ROOT/'results/unified_exact_round86/publication_api'
TEMP=ROOT/'build/round86/publication_api'
GH='C:/Program Files/GitHub CLI/gh.exe'
REPO='repos/yifanXovo/TailoredExact'
BRANCH='codex/round86-ensc-varied-unadapted-confirmation'
...
def main():
    ...
    try:
        ...
        if existing is None:
            initial='a3a7c89cfa752fd0c029c83d43996562df0be3fd'
        ...
        # Git content addresses bind uploaded bytes without claiming a second
        # full-body download. Local evidence bundles are byte-verified separately.
        record['binary_verification_policy']='Each local Git blob SHA is recomputed from bytes and matched to the base64 creation response; exact root tree and commit required. No full remote body readback is claimed.'
        save()
        binary_verified=set()
        def exact_binary_blob(digest,data):
            ...
            record.setdefault('binary_content_addresses',[]).append(dict(sha=digest,bytes=len(data),sha256=sha256,
                verification='current_creation_SHA_acknowledgment_no_full_readback'))
            save();return digest
        def exact_tree(previous,target_tree):
            ...
            # Text is inline; binary entries reference creation-SHA-verified blobs.
