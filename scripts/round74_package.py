...
def main():
    ...
    manifest=dict(schema='round74-lossless-bundles-v1',bundles=bundles,
    ...
    delivery=dict(all_members_hash_verified=True,bundles=len(bundles),
        ...
        verification_command='python scripts/round74_package.py --verify-only')
    ...
    write(OUT.parent/'resource_summary.json',dict(inherited_ctests=54,newly_executed_ctests=0,
        new_builds=0,new_native_fixture_calls=0,new_full_runs=4,
        actual_full_optimize_calls=audit['actual_optimize_calls'],
        full_run_wall_seconds=summary['total_wall_seconds'],
        offline_replay_seconds=summary['offline_replay_seconds'],
