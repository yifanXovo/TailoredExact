...
from round86_analyze import campaign_summary
...
def main():
    ...
    for launch,done in zip(frozen['launches'],summary['records']):
        ...
        if done['audit']['endpoint']['U'] is None:
            assert arm=='P-GRB' and not done['audit']['endpoint']['certificate']
            row['endpoint_physical']=None
            row['no_physical_UB']=True
        elif done['stop_reason']=='normal_return':
        ...
        if arm not in ['BDS-C','ENS-C']:
            ...
            mathematical_zero=row['neutral_physical_audit']['initial']['F']<=1e-12
        ...
        arms.append(row)
    assert len(traces)==4 and controls==8
    assert not frozen['prior_bindings'] and not read(OUT.parent/'protocol.json')['prior_bindings']
    output=dict(all_checks_passed=True,arms=arms,start_decisions=starts,controls_checked=controls,
        prior_startup_pairs_checked=0,prior_startup_pairs=[],
        all25_paths_match_available_prior_startup_screen=None,
