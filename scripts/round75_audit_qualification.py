...
from round75_qualify import ROOT,OUT,read,write,sha

def main():
    version=sys.argv[1];started=time.perf_counter()
    destination=OUT/f'qualification_{version}_audit.json'
    assert not destination.exists()
    build=ROOT/'build/round75'/version;q=read(OUT/f'qualification_{version}.json')
    ...
    passed='100% tests passed, 0 tests failed out of 56' in text
    ...
    total=sum(logs.values());ceiling=90 if version=='v1' else 120
    assert total<=ceiling
    failed_cli=[]
    for path in build.glob('round75_quantity_cli_*/*/result.json'):
        result=read(path)
        if not result['strict_certified_original_problem']:
            assert version=='v1' and result['external_gini_tree_failure_reason']=='c6_startup_variant_contract_mismatch_or_local_redecode'
            failed_cli.append(dict(path=str(path.relative_to(ROOT)),sha256=sha(path),status=result['status'],
                reason=result['external_gini_tree_failure_reason'],UB=result['upper_bound'],native_calls=0))
    if passed:assert not failed_cli
    oracle=re.search(r'Round75 oracle comparisons=.*',text)[0]
    report=dict(version=version,passed=passed,tests_total=56,tests_passed=56 if passed else 55,
