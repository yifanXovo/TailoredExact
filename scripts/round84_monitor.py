...
out=dict(id=read(folder/'launch.json')['panel']['id'],arm=a['arm'],launch=a['number'],elapsed_seconds=round(time.time()-a['started_unix'],1),
...
if (OUT/'summary.json').exists():
    ...
    out['completed_endpoints']=[dict(id=r['id'],arm=r['arm'],valid=r['audit']['passed'],wall=r['wall_seconds'],endpoint=r['audit'].get('endpoint')) for r in s['records'][-3:]]
