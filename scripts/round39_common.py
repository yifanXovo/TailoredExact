#!/usr/bin/env python3
"""Shared frozen identities, commands, and IO helpers for Round 39."""
...
OUT = ROOT / "results" / "gf_small_hard_light_round39"
RUNS = OUT / "runs"
INVALIDATED = OUT / "invalidated_rows"
BUILD = ROOT / "build_round39" / "official" / "gurobi"
EXE = BUILD / "ExactEBRP.exe"
INSTANCE_MANIFEST = OUT / "frozen_instance_manifest.csv"
DESCRIPTOR_TABLE = OUT / "structural_descriptor_table.csv"
OFFICIAL_MATRIX = OUT / "round39_official_matrix.csv"
GUARD_MANIFEST = OUT / "full_guard_subset_manifest.csv"
FINGERPRINTS = OUT / "gurobi_fingerprints.json"
FROZEN_MANIFEST = OUT / "round39_frozen_manifest.json"
COMMAND_FREEZE = OUT / "round39_command_freeze.json"
SHUTDOWN_MARGIN = 20
PROCESS_CAP_SECONDS = 21600
...
def plain_command(item: dict[str, Any], run_dir: Path,
    ...
    for name, value in (
        ...
        ("--round24-expected-gurobi-model-fingerprint", expected_fingerprint),
...
def c6_command(item: dict[str, Any], arm: str, run_dir: Path,
    ...
    if arm not in {"C6-HGA-LIGHT-1000", "C6-HGA-FULL"}:
        raise ValueError(f"unknown C6 arm: {arm}")
    light = arm == "C6-HGA-LIGHT-1000"
    args = [str(EXE), "--input", str(item_path(item))]
    ...
    for name, value in (
        ...
        ("--primal-heuristic-no-improve-generations", 1000 if light else 2000),
    ):
        replace(args, name, value)
    for name, value in (
        ("--round34-c6-startup-variant",
         "hga-light-1000" if light else "hga-full"),
...
def command_for(row: dict[str, str], item: dict[str, Any],
    ...
    if row["arm"] == "P-GRB":
        return plain_command(item, run_dir, fingerprint, process_cap=cap)
    return c6_command(item, row["arm"], run_dir, fingerprint, process_cap=cap)


def required_artifacts(arm: str, run_dir: Path) -> list[Path]:
    common = [run_dir / "command.json", run_dir / "result.json",
