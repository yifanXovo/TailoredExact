"""Round78 frozen D7 BDS-C complete-method screen on qualified production bytes."""
import json
import os
...
STAGE=ROOT/'results/unified_exact_round78'
...
def main():
    ...
    assert not subprocess.check_output(['git','status','--porcelain','--','src','include','tests','CMakeLists.txt','scripts/round78_research.py','results/unified_exact_round78/full_screen_plan.md'],cwd=ROOT).strip(), 'Freeze measured source and driver before launch'
    startup=read(STAGE/'startup/summary.json')
    ...
    roles=[(panel['D7'],arm,False) for arm in ['P-GRB','BDS-C','K1-R']]
    ...
    for p in [panel['D7']]:
        ...
        write(ref/'launch.json',dict(command=command,binary_sha256=sha(build/'Round65ReferenceBuild.exe'),
            input=p,cap=30,expected_optimize_calls=0,plan_sha256=sha(STAGE/'full_screen_plan.md')))
    ...
    identity = dict(source_commit=frozen['source_commit'],binary_sha256=sha(build/'ExactEBRP.exe'),
        ...
        physical_sha256=sha(evidence.physical_module.__file__),plan_sha256=sha(STAGE/'full_screen_plan.md'),
        ...
        for launch in launches:
            ...
            try:
                ...
                if launch['arm']=='BDS-C':
                    ...
                    prior=normalize(read(ROUND78/'startup/local_raw/D7/BDS-C/hga.csv.balanced/initial.json'))
                    ...
                    previous=ROUND78/'startup/local_raw/D7/BDS-C/hga.csv.descent.csv'
