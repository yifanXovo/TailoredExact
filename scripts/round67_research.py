"""Frozen Round67 serial protocol; all deadlines terminate the complete run."""
import argparse, json, os, subprocess, time
from pathlib import Path
import round61_research as runner

ROOT=Path(__file__).resolve().parents[1]
OUT=ROOT/'results/unified_exact_round67';BUILD=ROOT/'build/round67';RAW=OUT/'local_raw'
...
def freeze():
    ...
    old=read(ROOT/'results/unified_exact_round66/protocol.json')['panel']
    ...
    write(OUT/'protocol.json',dict(base='7c3b18912e16c66f9927ff71563fc25a9f488ff5',panel=selected,
        plan_sha256=sha(OUT/'plan.md'),mathematics_sha256=sha(OUT/'mathematics.md'),
        resource_revision_sha256=sha(OUT/'resource_plan_update_1.json'),
        maximum_performance=16,maximum_micro=6,worst_case_seconds=6120,
        ...
        candidate='K1-R plus logarithmic VD-P; ARC and all resource policies off',
        arms=['P-GRB','K1-R','VD-P','LOG'],confirmation_opened=False))
    ...
    write(OUT/'build_v1.json',dict(source_commit=subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip(),
        ...
        tests=sha(BUILD/'tests.log'),driver=sha(Path(__file__)),shared_command_helper=sha(Path(runner.__file__))))
...
def refbuild(ids):
    ...
    previous=read(ROOT/'results/unified_exact_round66/fingerprints.json')
    for identity in ids:
    ...
    for identity in ids:
        ...
        for arm in arms:
            ...
            if arm=='P-GRB':
                ...
                preset={'K1-R':'research-round65-k1-h','VD-P':'research-round67-vdp','LOG':'research-round67-log-vdp'}[arm]
...
if __name__=='__main__':
    ...
    ap.add_argument('--ids',nargs='+',default=[]);ap.add_argument('--arms',nargs='+',default=['P-GRB','K1-R','VD-P','LOG'])
