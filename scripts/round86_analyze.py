...
OUT=ROOT/'results/unified_exact_round86/campaign'
...
def endpoint(done):
    ...
    ep=done['audit']['endpoint']
    return gap_fields(dict(ep,available=ep['U'] is not None,paid_completion_seconds=done['wall_seconds']))


def checkpoint(records,done,t):
    if done['wall_seconds']<=t and done['stop_reason']=='normal_return':
        return gap_fields(dict(done['audit']['endpoint'],available=done['audit']['endpoint']['U'] is not None,
...
def compare(a,b,V):
    if not a['available'] or not b['available']:
        classification = 'certificate_loss_missing_candidate_UB' if a['certificate'] else 'certificate_gain_missing_reference_UB' if b['certificate'] else 'unavailable_physical_endpoint'
        return dict(classification=classification,reference_upper_available=a['available'],candidate_upper_available=b['available'],LB_delta=b['L']-a['L'],material_improvement=None,material_regression=None,severe_regression=None)
...
def checkpoint_times(cap):
    return {120:[30,60,120],300:[60,120,180,300],600:[60,120,300,600],3600:[300,600,1200,1800,2400,3600]}[cap]
...
def main():
    ...
    assert summary['completed']==12 and all(r['audit']['passed'] for r in summary['records'])
    assert sha(ROOT/'scripts/round86_research.py')==frozen['driver_sha256']
    assert sha(evidence.__file__)==frozen['reader_sha256']
    assert sha(evidence.physical_module.__file__)==frozen['physical_sha256']
    assert sha(ROOT/'build/round83/v1/ExactEBRP.exe')==frozen['binary_sha256']
    assert sha(ROOT/'results/unified_exact_round86/plan.md')==frozen['plan_sha256']
    for name,digest in frozen['source'].items():assert sha(ROOT/name)==digest,name
    assert sha(ROOT/'results/unified_exact_round83/startup/completion_summary.json')==frozen['startup_summary_sha256']
    assert sha(ROOT/'scripts/round83_audit_v2.py')==frozen['exchange_replay_sha256']
    assert sha(OUT.parent/'protocol.json')==frozen['protocol_sha256']
    for name in ['preregistration','generation']:
        assert sha(OUT.parent/(name+'.json'))==frozen[name+'_sha256']
    assert sha(OUT.parent/'offline_validation_v2.json')==frozen['offline_validation_sha256']
    assert sha(ROOT/'build/round83/v1/Round65ReferenceBuild.exe')==frozen['reference_binary_sha256']
    qualification=read(ROOT/'results/unified_exact_round83/qualification_v1.json')
    assert sha(ROOT/'CMakeLists.txt')==qualification['identity']['cmake_sha256']
    assert len(frozen['launches'])==12 and len(summary['records'])==12
    all_checkpoints=[];actual_calls=[];endpoint_checks=[];observation_coverage=[]
    for launch,done in zip(frozen['launches'],summary['records']):
        assert (done['number'],done['id'],done['arm'])==(launch['number'],launch['panel']['id'],launch['arm'])
        ...
        if done['stop_reason']=='normal_return':
            ...
            if records:
                evidence.finalize_endpoint(ROOT,launch['panel'],launch['arm'],replay,records,result,done['stop_reason'],frozen['references'][launch['panel']['id']])
                assert replay['endpoint']==endpoint
            if endpoint['U'] is not None:
                ...
                assert records and launch['arm']=='P-GRB'
                assert not endpoint['certificate'] and endpoint['gap'] is None
                assert not result['verified_incumbent_objective_available']
                assert replay['physical_witnesses']==0
    ...
    for identity in ['F1','F2','F5','F6']:
        ...
        for t in checkpoint_times(representative['cap']):
            ...
            pairs=[('P-GRB','ENS-C')]
            if 'BDS-C' in arms:pairs += [('P-GRB','BDS-C'),('BDS-C','ENS-C'),('K1-R','BDS-C')]
            if 'K1-R' in arms:pairs += [('P-GRB','K1-R'),('K1-R','ENS-C')]
            for a,b in pairs:
                comparisons.append(dict(id=identity,seconds=t,reference=a,candidate=b,**compare(arms[a],arms[b],V)))
            p,j=arms['P-GRB'],arms['ENS-C'];k=arms.get('K1-R')
            ...
            protection.append(dict(id=identity,seconds=t,fresh_K1_control_available=k is not None,
                ...
                scope='Fresh same-build prospectively frozen confirmation comparison only; all controls are current. No algorithm gate or historical-fastest veto'))
    assert len(comparisons)==57 and len(protection)==19
    ...
    audit=dict(all_checks_passed=True,completed=12,actual_optimize_calls=len(actual_calls),
