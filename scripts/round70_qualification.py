...
def main():
    ...
    assert '100% tests passed, 0 tests failed out of 46' in tests
    assert run.sha(run.BUILD/'test_1.log')==run.sha(run.BUILD/'tests.log')
    files=[run.ROOT/'CMakeLists.txt']+[p for d in ['src','include','tests'] for p in (run.ROOT/d).rglob('*') if p.suffix in ['.cpp','.hpp','.h','.c']]
    for p in (run.ROOT/'scripts').glob('*round70*.py'):ast.parse(p.read_text(encoding='utf-8'),filename=str(p))
    run.write(destination,dict(final_ctest_passed_tests=46,final_binary_sha256=run.sha(run.BUILD/'ExactEBRP.exe'),
    ...
    print('Qualified46 fresh tests, source files',len(files),'binary',run.sha(run.BUILD/'ExactEBRP.exe'))
