...
from round75_qualify import ROOT, OUT as STAGE, sha, read, write
OUT=STAGE/'startup'
...
def replay_quantity(p,dest,baseline,result):
    initial=normalize(read(dest/'hga.csv.quantity.csv.initial.json'))
    final=normalize(read(dest/'hga.csv.quantity.csv.final.json'))
    assert route_hash(initial)==route_hash(baseline),'Refinement must begin at the unchanged best JDS-X physical witness'
    checked=physical_module.physical(p,initial)
    assert checked['original_T_feasible']
    rows=csvrows(dest/'hga.csv.quantity.csv');assert rows[0]['status']=='initial_verified'
    assert rows[-1]['status']=='neighborhood_exhausted','Deadline/rejection is not exhausted quantity descent'
    routes=initial['routes'];prior=checked['F'];last_time=-1;accepted=0
    counts=dict(singles=0,pairs=0,cross_vehicle=0,sign_flips=0,removed_stops=0)
    witnesses=1
    for row in rows:
        ...
        counts['pairs' if b else 'singles']+=1
        if b:counts['cross_vehicle']+=owners[a]!=owners[b]
        checked=physical_module.physical(p,dict(routes=routes,objective=float(row['objective'])))
    ...
    if not returned_to_outer:
        # The unchanged outer incumbent store uses1e-10, while this heuristic
        # uses1e-12. A smaller gain may retain the original witness honestly.
    ...
    return dict(initial_UB=initial['F'],final_UB=final['F'],accepted=accepted,counts=counts,
        ...
        passes=int(rows[-1]['passes']),single_candidates=int(rows[-1]['single_candidates']),
        pair_candidates=int(rows[-1]['pair_candidates']),feasible_candidates=int(rows[-1]['feasible_candidates']),
...
def main():
    version=sys.argv[1];build=ROOT/'build/round75'/version
    ...
    assert '100% tests passed, 0 tests failed out of 56' in (build/'tests.log').read_text(encoding='utf-8')
    ...
    for p in selected:
        ...
        for arm,preset in [('JDS-X','research-round73-vds-joint-seeded-descent'),('QDS-X','research-round75-vds-quantity-descent')]:
    ...
    try:
        for command in commands:
            ...
            if command['arm']=='QDS-X':
                ...
                detail.update(replay_quantity(p,dest,baseline,result))
                detail['all25_logical_descent_paths_equal']=True
                assert physical['F']<=baseline['upper_bound']+1e-10
            else:
                assert not (dest/'hga.csv.quantity.csv').exists()
