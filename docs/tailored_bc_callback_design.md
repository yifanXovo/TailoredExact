...
This still is not the full hard-leaf tailored BC design. Candidate validation is compact-row consistency validation, not independent route-plan verification through the ExactEBRP evaluator. Custom Gini branch creation is observed only in a diagnostic toy MIP and is not yet validated on ExactEBRP hard leaves. The callback-separated visit-inventory, Gini subset-envelope, and low-Gini L1 cuts share the same proofs as the static rows, but they have only been exercised in smoke diagnostics so far.
