#!/usr/bin/env python3
"""Losslessly compress, scan, inventory, and verify Round 35 evidence."""
...
def candidates() -> list[Path]:
    output = []
    for root in (common.RUNS, common.INVALIDATED):
        if not root.exists():
            continue
...
def compress(path: Path) -> dict[str, Any]:
    original_bytes, original_sha = path.stat().st_size, sha256(path)
    target, temporary = Path(str(path) + ".gz"), Path(str(path) + ".gz.tmp")
...
def package_files() -> list[Path]:
    ...
    return sorted((path for path in OUT.rglob("*")
...
def main() -> int:
    audit_path, report_path = (
        OUT / "final_audit_summary.json", OUT / "final_report.md")
    if not audit_path.is_file() or not report_path.is_file():
        raise RuntimeError("Round 35 final analysis must run before packaging")
    if len(list(common.RUNS.glob("*/completion_marker.json"))) != 52:
        raise RuntimeError("all 52 official completion markers are required")
    lock = OUT / ".round35_runner.lock"
    if lock.exists():
        raise RuntimeError("cannot package while the official runner is active")
    ...
    oversized = [common.relative(path) for path in files
                 if path.stat().st_size >= 100 * 1024 * 1024]
    if oversized:
        raise RuntimeError(f"artifacts at or above 100 MiB: {oversized}")
    ...
    audit.update({
        ...
        "artifacts_at_or_above_100_mib": 0,
    ...
    for _ in range(8):
        ...
        write_text(report_path, base_report + f"""
...
largest retained artifact is `{common.relative(largest)}`
...
""")
        ...
        if len(updated) == len(files) and sum(
                path.stat().st_size for path in updated) == total_bytes:
            break
    else:
    ...
    rows = [{
        ...
        if any(part in {"runs", "invalidated_rows"} for part in path.parts)
