#!/usr/bin/env python3
"""Run frozen pre/post default-off equivalence pairs for Round 41."""
...
def manifest(phase: str) -> list[dict[str, Any]]:
    ...
    for instance_id in PANEL:
        ...
        for arm in ARMS:
            ...
            rows.append({
                "round_id": 41,
                "audit": f"{phase}_default_c6_equivalence",
                "phase": phase,
                "serial_order": serial,
                "run_id": f"{phase}_default_equivalence__{instance_id}__{suffix}",
...
def run_one(row: dict[str, Any], force: bool) -> None:
    ...
    record = {
        "schema": "round41-default-equivalence-run-v1",
        **row,
        "command": command,
...
def main() -> int:
    ...
    common.write_csv(
        ...
        rows)
    start_path = common.OUT / f"{args.phase}_default_c6_equivalence_start.json"
    if not start_path.exists():
        common.write_json(start_path, {
            "schema": "round41-default-equivalence-start-v1",
