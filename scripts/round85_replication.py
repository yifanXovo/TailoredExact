"""One same-build D6 repetition, preserving both realizations; no optimization."""
import json
import time
from round85_analyze import ROOT, OUT, read, compare, sha, write, cross_run_consistency
...
def main():
    ...
    prior=ROOT/'results/unified_exact_round84/campaign'
    assert sha(prior/'identity.json')==protocol['prior_D6_campaign_identity_sha256']
    assert sha(prior/'endpoint_checks.json')==protocol['prior_D6_endpoints_sha256']
    ...
    assert summary['completed']==21 and old_summary['completed']==9
    ...
    for arm in arms:
        launch=next(l for l in current['launches'] if l['panel']['id']=='D6' and l['arm']==arm)
        old_launch=next(l for l in previous['launches'] if l['panel']['id']=='D6' and l['arm']==arm)
        ...
        done=next(r for r in summary['records'] if r['id']=='D6' and r['arm']==arm)
        old_done=next(r for r in old_summary['records'] if r['id']=='D6' and r['arm']==arm)
        old_end=endpoint(old_done);new_end=endpoint(done)
        old_ends[arm]=old_end;new_ends[arm]=new_end
        repeated.append(dict(id='D6',arm=arm,prior_stage=84,current_stage=85,cap=3600,
            prior=old_end,current=new_end,change_between_realizations=compare(old_end,new_end,30),
        ...
        for stage,row in [(84,old_end),(85,new_end)]:
            consistency_records.append(dict(id='D6',arm=arm,origin=stage,**row))
        for t in [300,600,1200,1800,2400,3600]:
            old=next(r for r in old_points if r['id']=='D6' and r['arm']==arm and r['seconds']==t)
            new=next(r for r in fresh_points if r['id']=='D6' and r['arm']==arm and r['seconds']==t)
            points.append(dict(id='D6',arm=arm,seconds=t,prior=old,current=new,
                descriptive_change=compare(old,new,30)))
            consistency_records.extend([dict(old,origin=84),dict(new,origin=85)])
    for a,b in [('P-GRB','ENS-C'),('P-GRB','K1-R'),('K1-R','ENS-C')]:
        paired.append(dict(id='D6',seconds=3600,reference=a,candidate=b,
            prior_comparison=compare(old_ends[a],old_ends[b],30),
            current_comparison=compare(new_ends[a],new_ends[b],30)))
        for t in [300,600,1200,1800,2400]:
            pair={arm:next(r for r in points if r['arm']==arm and r['seconds']==t) for arm in [a,b]}
            paired.append(dict(id='D6',seconds=t,reference=a,candidate=b,
                prior_comparison=compare(pair[a]['prior'],pair[b]['prior'],30),
                current_comparison=compare(pair[a]['current'],pair[b]['current'],30)))
    ...
    write(OUT/'replication.json',dict(realizations=repeated,checkpoints=points,matched_pair_comparisons=paired,
        scope='Every pair uses controls from its own realization. Other roles have fresh matched current-build controls; earlier-build rows remain historical context.'))
