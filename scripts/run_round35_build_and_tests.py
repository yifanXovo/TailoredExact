#!/usr/bin/env python3
"""Create the isolated Round 35 Release build and run all regressions."""
...
def write_records(records: list[dict[str, Any]]) -> None:
    common.write_json(common.OUT / "stage0_build_and_tests.partial.json", records)
...
def execute(name: str, command: list[str],
    ...
    if completed.returncode != 0:
        raise RuntimeError(f"Round 35 stage-0 command failed: {name}")


def main() -> int:
    common.OUT.mkdir(parents=True, exist_ok=True)
    if common.BUILD.exists():
        raise SystemExit("Round 35 build directory already exists; clean build required")
    ...
    try:
        ...
        execute("stage0_build_gurobi",
                [str(CMAKE), "--build", str(common.BUILD), "-j", "2"], records)
        ...
        for test in sorted((common.ROOT / "tests").glob("*.py")):
            execute(f"stage0_python_{test.stem}",
                    [str(PYTHON), "-B", str(test)], records)
    except Exception as error:
        common.write_json(common.OUT / "stage0_build_and_tests.json", {
            "schema": "round35-build-tests-v1", "round_id": 35,
            "source_commit": head, "records": records,
            "passed": False, "failure": str(error),
        })
        raise
    summary = {
        "schema": "round35-build-tests-v1",
        "round_id": 35,
        "source_commit": head,
    ...
    (common.OUT / "stage0_build_and_tests.partial.json").unlink(missing_ok=True)
    print(json.dumps({
