...
This user-authorized continuation is an explicit operational exception to the frozen `reproduce.md` instruction to put a fresh post-interruption campaign in a separate root. The frozen solver protocol, executable, inputs, arm identities, order, and per-arm caps are unchanged; the existing root is extended only with previously unstarted arms. No two process segments are spliced into one arm. Run 8 ended before the 86,400-second cap, so its censoring is administrative, not evidence of failure to certify within the full budget. Any offline comparison label implying a full-budget advantage for F2 must be qualified accordingly.
