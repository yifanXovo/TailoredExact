"""Plot audited U6 U/L/gap on the common four-arm whole-run timeline."""
import hashlib
import json
import math
import time
...
def main():
    ...
    points = [r for r in read(campaign / 'checkpoints.json') if r['id'] == 'U6']
    ...
    plt.rcParams.update({'font.family': 'DejaVu Sans', 'font.size': 10,
                         'svg.fonttype': 'none', 'svg.hashsalt': 'round83-frozen-checkpoints'})
    colors = {'P-GRB': '#575E68', 'BDS-C': '#C45112', 'K1-R': '#2267A5', 'ENS-C': '#009E73'}
    fig, axes = plt.subplots(1, 3, figsize=(15, 5.4))
    for axis, metric, title in zip(axes, ['U','L','gap'],
                                    ['Verified original upper bound U',
                                     'Qualified whole-domain lower bound L',
                                     'Absolute gap U - L']):
        identity='U6'
        values = []
        arms = ['P-GRB', 'BDS-C', 'ENS-C', 'K1-R']
        for arm in arms:
    ...
    fig.legend(handles, labels, loc='upper center', bbox_to_anchor=(0.52, 0.928),
               ncol=4, frameon=False, handlelength=2.3, columnspacing=1.8)
    fig.suptitle('Round83: U6 four-arm comparison at a common 3600-second cap', fontsize=14, y=0.98)
    fig.subplots_adjust(left=0.075, right=0.985, top=0.82, bottom=0.29, wspace=0.30)
    fig.text(0.075, 0.11,
             'Markers are audited checkpoints; dashed lines only guide the eye. Missing U leaves U/gap blank; L is shown as recorded.\n'
             'Whole elapsed time includes startup, all native calls and exit. An outlined square marks a valid hard-stop endpoint if present.\n'
             'U6: V50/M4/Q30, mathematical T18000; exposed development on one common qualified build.',
             fontsize=9, va='top', linespacing=1.5)
    destination.mkdir()
    fig.savefig(destination / 'u6_bounds_and_gap.svg', metadata={'Date': None})
    fig.savefig(destination / 'u6_bounds_and_gap.png', dpi=180)
    plt.close(fig)
    (destination / 'plotted_checkpoints.json').write_text(json.dumps(points, indent=2) + '\n', encoding='utf-8')
    outputs = [destination / name for name in ['u6_bounds_and_gap.svg', 'u6_bounds_and_gap.png',
                                               'plotted_checkpoints.json']]
    receipt = dict(layout_revision=1, data_source='completed_audited_original_campaign', optimizer_calls=0, wall_seconds=time.perf_counter() - started,
