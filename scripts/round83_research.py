"""Round83 frozen same-build complete-method exchange screen."""
import json
import os
...
STAGE=ROOT/'results/unified_exact_round83'
ROLE_IDS=['E8','N12','D4','U6']
CAPS=dict(E8=120,N12=120,D4=300,U6=3600)
ARMS={i:['P-GRB','ENS-C','K1-R'] for i in ROLE_IDS};ARMS['U6']=['P-GRB','BDS-C','ENS-C','K1-R']
...
def main():
    ...
    assert not subprocess.check_output(['git','status','--porcelain','--','src','include','tests','CMakeLists.txt','scripts/round83_research.py','results/unified_exact_round83/full_screen_plan.md','results/unified_exact_round83/protocol.json'],cwd=ROOT).strip(), 'Freeze source and driver first'
    startup=read(STAGE/'startup/completion_summary.json')
    assert startup['attempted']==startup['valid']==10 and startup['optimize_calls']==0
    assert read(STAGE/'base_publication.json')['head_sha']=='30996a5c023adc2257cf9f4581e3672cf9a644d1'
    ...
    assert len(roles)==13 and sum(CAPS[p['id']] for p,_,_ in roles)==16020
    ...
    identity = dict(source_commit=frozen['source_commit'],binary_sha256=sha(build/'ExactEBRP.exe'),
        ...
        launches=launches,references=references,prior_bindings={},startup_summary_sha256=sha(STAGE/'startup/completion_summary.json'),protocol_sha256=sha(STAGE/'protocol.json'),maximum_process_seconds=16020,formal_performance=True)
    ...
    try:
        for launch in launches:
            ...
            write(OUT/'summary.json',dict(records=records,total_wall_seconds=sum(r['wall_seconds'] for r in records),
                ...
                completed=len(records),planned=13,formal_performance=True))
        ...
        lock.unlink()
        write(OUT/'driver_completion.json',dict(completed=len(records),planned=13,all_valid=len(records)==13 and all(r['audit']['passed'] for r in records),ended_unix=time.time()))
