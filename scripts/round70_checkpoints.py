...
def link(folder,p,result,overhead=0):
    if result.get('hga_stop_mode')!='decoded-descent':
...
def main():
    ...
    for r in records:
        ...
        if r['arm']=='DS' and 'generation-completion' in r.get('UB_scope',''):
    ...
    scope.update(round70_wrapper_sha256=run.sha(__file__),hga_provenance_helper_sha256=run.sha(hga.__file__),
        ...
        horizons='300 and600 for each declared D6/D7 arm; no new run or internal stopping rule',
