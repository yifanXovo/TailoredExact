#!/usr/bin/env python3
"""Verify that the new K1-AMF mode is behaviorally inert when off."""
...
def main() -> int:
    ...
    for mode in ("implicit", "explicit"):
        ...
        if mode == "implicit":
            common.remove_option(command, "--round48-k1-amf")
        common.write_json(run_dir / "command.json", {
            "schema": "round48-default-off-command-v1", "mode": mode,
            "round48_k1_amf": "implicit-default" if mode == "implicit" else "off",
            "command": command, "executable_sha256": common.sha256(executable),
    ...
    common.write_json(common.OUT / "default_off_equivalence.json", {
        "schema": "round48-default-off-equivalence-v1", "passed": passed,
        "instance": INSTANCE, "process_cap_seconds": args.process_cap,
        "executable_sha256": common.sha256(executable),
        "historical_mode": "K1-AM", "correctness_sentinel_not_benchmark_row": True,
        "implicit_command_omits_round48_option":
            "--round48-k1-amf" not in commands["implicit"],
        "explicit_command_supplies_off":
            commands["explicit"][commands["explicit"].index(
                "--round48-k1-amf") + 1] == "off",
    })
