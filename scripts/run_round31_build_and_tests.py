#!/usr/bin/env python3
"""Create clean Round 31 builds and run every C++ and Python regression."""
...
BUILD_ROOT = ROOT / "build_round31/official"
CPLEX_BUILD = BUILD_ROOT / "cplex_r1"
GUROBI_BUILD = BUILD_ROOT / "gurobi_r1"
CMAKE = Path(
...
def write_csv(path: Path, rows: list[dict[str, Any]]) -> None:
    with path.with_suffix(path.suffix + ".tmp").open(
...
def main() -> int:
    if CPLEX_BUILD.exists() or GUROBI_BUILD.exists():
        raise SystemExit(
            "clean official build destinations already exist; audit them")
    ...
    summary = {
        ...
        "cplex_executable":
            cplex_exe.relative_to(ROOT).as_posix(),
        "cplex_executable_sha256": sha256(cplex_exe),
        "gurobi_executable":
            gurobi_exe.relative_to(ROOT).as_posix(),
        "gurobi_executable_sha256": sha256(gurobi_exe),
        "cpp_test_count_per_build": 14,
        "python_test_script_count":
            len(tuple((ROOT / "tests").glob("*.py"))),
        "records": records,
