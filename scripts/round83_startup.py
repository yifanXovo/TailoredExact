...
from round83_audit import audit as replay_exchange
OUT=STAGE/'startup'
...
def main():
    version=sys.argv[1];build=ROOT/'build/round83'/version
    ...
    selected=[x['input'] for x in read(STAGE/'diagnostic/v1/identity.json')['launches'] if x['id']!='structural']
    commands=[]
    for p in selected:
        assert sha(ROOT/p['instance_path'])==p['input_sha256']
        for arm,preset in [('BDS-C','research-round78-vds-balanced-descent'),('ENS-C','research-round83-vds-equal-net-exchange')]:
            ...
            command=list(map(str,[build/'ExactEBRP.exe','--input',p['instance_path'],'--lambda',p['lambda'],
                ...
                '--time-limit',54,'--process-wall-time-limit',60,'--process-shutdown-margin',3,
            ...
            commands.append(dict(number=len(commands)+1,id=p['id'],arm=arm,command=command,input=p,
                destination=str(dest.relative_to(ROOT)),cap=60,hard_deadline=59.8,expected_optimize_calls=0))
    identity=dict(source_commit=frozen['source_commit'],source=frozen['source'],binary_sha256=sha(build/'ExactEBRP.exe'),
        ...
        balanced_replay_sha256=sha(ROOT/'scripts/round78_audit.py'),exchange_replay_sha256=sha(ROOT/'scripts/round83_audit.py'),
        closure_replay_sha256=sha(ROOT/'scripts/round76_startup.py'),
        commands=commands,max_launches=10,maximum_seconds=600,expected_optimize_calls=0,
    ...
    try:
        for command in commands:
            ...
            assert code==0 and not watchdog and completion['wall_seconds']<=60
