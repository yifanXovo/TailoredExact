...
def main():
    ...
    closure=list(build.glob('round83_exchange_cli_*'));control=list(build.glob('round78_balanced_cli_*'))
    ...
    records=[]
    for arm,root in [('ENS-C',closure[0]),('BDS-C',control[0])]:
        folder=root/arm;result=read(folder/'result.json')
        if arm=='BDS-C':
            assert not (folder/'hga.csv.exchange').exists()
            assert 'Round83 equal-net exchange descent' not in '\n'.join(result.get('notes',[]))
