"""Three role plots from completed audited checkpoints only; no optimizer."""
import hashlib
import json
import math
import time
...
OUT=ROOT/'results/unified_exact_round84'
...
def main():
    ...
    assert len(points)==54 and set(panel)=={'D6','D7','U6'}
    for row in points:
    ...
    plt.rcParams.update({'font.family':'DejaVu Sans','font.size':10,
        'svg.fonttype':'none','svg.hashsalt':'round84-frozen-checkpoints'})
    ...
    for identity in ['D6','D7','U6']:
        ...
        for axis,metric,title in zip(axes,['U','L','gap'],
            ...
            for arm,color in colors.items():
                ...
                if end['stop_reason']!='normal_return' and rows[-1]['available']:
                    axis.scatter([3600],[y[-1]],s=85,marker='s',facecolors='none',edgecolors=color,linewidths=1.5,zorder=5)
            axis.set(title=title,xlabel='Whole-run elapsed time (seconds)',ylabel=metric)
            axis.set_xticks([300,600,1200,1800,2400,3600]);axis.tick_params(axis='x',labelrotation=35)
        ...
        fig.suptitle(f'Round84: {identity} comparison at a common 3600-second cap',fontsize=14,y=.98)
    ...
    receipt=dict(layout_revision=1,roles=['D6','D7','U6'],data_source='completed_audited_original_campaign',
