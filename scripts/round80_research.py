"""Round80 frozen E8 and common3600 D6 primary-deficit validation."""
import json
import os
...
STAGE=ROOT/'results/unified_exact_round80'
ROLE_IDS=['E8','D6']
CAPS=dict(E8=120,D6=3600)
...
def main():
    ...
    assert subprocess.check_output(['git','branch','--show-current'],cwd=ROOT).decode().strip()=='codex/round80-bdsc-primary-deficits'
    assert not subprocess.check_output(['git','status','--porcelain','--','src','include','tests','CMakeLists.txt','scripts/round80_research.py','results/unified_exact_round80/plan.md'],cwd=ROOT).strip(), 'Freeze measured source and driver before launch'
    ...
    assert read(STAGE/'prior_publication_verified.json')['head_sha']=='c29173a6cb34c299cd818310eee59844ed36d8f6'
    ...
    old=read(ROOT/'results/unified_exact_round66/protocol.json')
    panel['E8']=dict(next(p for p in old['panel'] if p['id']=='E8'))
    panel['E8']['role']='Exposed current-K1 small regression, reclassified in R66; frozen R80 validation'
    panel['E8']['stage']='development'
    roles=[(panel[i],arm,False) for i in ROLE_IDS for arm in ['P-GRB','BDS-C','K1-R']]
    ...
    for p in [panel[i] for i in ROLE_IDS]:
        ...
        old_refs=read(ROOT/('results/unified_exact_round66/fingerprints.json' if p['id']=='E8' else 'results/unified_exact_round71/fingerprints.json'))
    ...
    identity = dict(source_commit=frozen['source_commit'],binary_sha256=sha(build/'ExactEBRP.exe'),
        ...
        launches=launches,references=references,maximum_process_seconds=11160,formal_performance=True)
    ...
    write(STAGE/'full_preflight.json',dict(inherited_source=frozen['source_commit'],
        ...
        reference_builds=2,reference_optimize_calls=0,reference_wall_seconds=reference_wall,
        ...
        for launch in launches:
            ...
            try:
                ...
                if launch['arm']=='BDS-C':
                    ...
                    prior_folder=ROUND78/'startup/local_raw'/launch['panel']['id']/'BDS-C'
            ...
            assert sum(r['audit'].get('native_calls_started',0) for r in records)<=150, 'Campaign resource ceiling; never an internal solver decision'
    finally:
        lock.unlink()
        write(OUT/'driver_completion.json',dict(completed=len(records),planned=6,all_valid=len(records)==6 and all(r['audit']['passed'] for r in records),ended_unix=time.time()))
