"""Publish exact existing commits using Git content addresses; distinguish byte readback from creation acknowledgments."""
import argparse
import base64
import datetime
import hashlib
...
def main():
    ...
    try:
        ...
        # Prior readback or creation receipts are content-addressed and bound to
        # this exact target, branch and unchanged v3 implementation. Large body
        # download failures do not become claims of successful byte readback.
        prior_byte_checks={};prior_creations=set()
        for name in ['final_evidence_binary.json','final_evidence_http1.json']:
            receipt=OUT/name
            if not receipt.exists():continue
            prior=json.loads(receipt.read_text(encoding='utf-8'))
            assert prior['target']==target and prior['branch']==BRANCH
            assert prior['script_sha256']=='24211f91182e540d1aa3a628cc66b02a5c0e57141069b98aab3d46c93f62d520'
            for item in prior.get('verified_binary_blobs',[]):
                prior_byte_checks[item['sha']]=item
            for item in prior.get('requests',[]):
                if item.get('method')=='POST' and item.get('path')=='git/blobs' and item.get('returncode')==0:
                    digest=item.get('returned_sha')
                    assert re.fullmatch('[0-9a-f]{40}',digest)
                    prior_creations.add(digest)
            record.setdefault('prior_receipt_bindings',[]).append(dict(file=name,sha256=hashlib.sha256(receipt.read_bytes()).hexdigest()))
        record['binary_verification_policy']='Git blob SHA equals locally recomputed content address; original tree and commit SHAs required. Prior successful byte readbacks are distinguished from creation-SHA acknowledgments. No new whole binary download is claimed.'
        save()
        binary_verified=set()
        def exact_binary_blob(digest,data):
            ...
            if digest in prior_byte_checks:
                previous=prior_byte_checks[digest]
                assert previous['bytes']==len(data) and previous['sha256']==sha256
                mode='prior_successful_full_byte_readback'
            elif digest in prior_creations:
                mode='prior_successful_creation_SHA_acknowledgment_no_full_readback'
            else:
                created=api('POST','git/blobs',dict(content=base64.b64encode(data).decode('ascii'),encoding='base64'))
                assert created['sha']==digest,'Original binary blob SHA mismatch; no ref update'
                mode='current_creation_SHA_acknowledgment_no_full_readback'
            binary_verified.add(digest)
            record.setdefault('binary_content_addresses',[]).append(dict(sha=digest,bytes=len(data),sha256=sha256,verification=mode))
            save();return digest
