"""Read-only solver evidence audit; compact outputs are written in the new stage."""
...
def tree_coverage(folder,leaves):
    ...
    return dict(tree_rows=len(leaves),roots=len(roots),replaced=sum(l['status']=='replaced' for l in leaves),
                independently_recomputed_interval_union=True)
def analyze():
    records=[];witnesses=[];calls_all=[];models=[];coverage=[];native_statistics=[]
    for e in run.runner.entries():
        ...
        if done['returncode'] or done['watchdog'] or not (folder/'result.json').exists():
            records.append(dict(number=e['charged_number'],id=e['id'],arm=e['arm'],failure=True,wall=done['wall_seconds']));continue
        ...
        if e['arm']=='P-GRB':
            ...
            if leaves:coverage.append(dict(number=e['charged_number'],id=e['id'],arm=e['arm'],**tree_coverage(folder,leaves)))
        ...
        for model in model_paths:
            ...
            if e['arm']=='ARC':
                assert 'r64_q_balance_' in text
                assert 'Generals\n' in text and 'Binaries\n' in text
                gen=text.partition('Generals\n')[2].partition('Binaries\n')[0]
                assert re.search(r'(?m)^\s*p_\d+_\d+\s*$',gen)
                assert not re.search(r'(?m)^\s*load_\d+_\d+\s*$',gen)
                assert 'r63f_' not in text
            if e['arm']=='Q-PLUS':
                assert 'r64_q_balance_' in text
                gen=text.partition('Generals\n')[2].partition('Binaries\n')[0]
            ...
            models.append(dict(number=e['charged_number'],path=str(model.relative_to(run.ROOT)),sha256=run.sha(model),
                bytes=model.stat().st_size,arc_q='r64_q_balance_' in text))
    ...
    pairs=[]
    for c in records:
        if c.get('failure') or c['arm']!='ARC':continue
        for b in records:
            if b.get('failure') or b['id']!=c['id'] or b['cap']!=c['cap'] or b['stage']!=c['stage'] or b['arm'] not in ['K1-R','P-GRB','Q-PLUS']:continue
            pairs.append(dict(id=c['id'],cap=c['cap'],candidate_number=c['number'],reference_number=b['number'],reference=b['arm'],
    ...
    run.write(run.OUT/'resource_status.json',dict(charged_completed=len(records),performance=sum(r.get('kind')=='performance' for r in records),
        ...
        ctest_batches=43,ctest_wall_seconds=4.40,ctest_note='Separate build qualification; any native unit-test calls are not added to experiment-call count',
        reference_build_only_runs=len(reference_runs),reference_build_wall_seconds=sum(x['wall_seconds'] for x in reference_runs),
        initial_performance_limit=run.read(run.OUT/'protocol.json')['initial_maximum_performance'],
        revised_performance_limit=max([run.read(run.OUT/'protocol.json')['initial_maximum_performance']]+[
            r.get('revised_performance_total',0) for r in revisions]),
        approved_worst_case_experiment_seconds=revisions[-1]['revised_stage_worst_case_seconds'] if len(revisions)>1 else 4660,
