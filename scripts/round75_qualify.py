...
OUT=ROOT/'results/unified_exact_round75'
...
def main():
    ...
    build=ROOT/'build/round75'/version
    assert not build.exists(),'Never replace a prior qualification attempt'
    assert subprocess.check_output(['git','branch','--show-current'],cwd=ROOT).decode().strip()=='codex/round75-physical-quantity-descent'
    assert not subprocess.check_output(['git','status','--porcelain','--','src','include','tests','CMakeLists.txt'],cwd=ROOT).strip(),'Commit measured source first'
    for stage in ['unified_exact_round72/campaign','unified_exact_round73','unified_exact_round74/campaign','unified_exact_round75/startup']:
    ...
    identity=dict(source_commit=subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT).decode().strip(),
        ...
        repair_plan_sha256=sha(OUT/'qualification_repair_plan.md') if version!='v1' else None,
        allowed_optimize_calls=90 if version=='v1' else 120)
