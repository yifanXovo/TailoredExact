"""Frozen ten-arm startup tranche. No native Optimize or imported witness."""
import ast
import csv
import hashlib
import json
...
def main():
    version='v3';build=ROOT/'build/round73'/version
    qualification=read(OUT/f'qualification_{version}.json')
    ...
    assert sha(OUT/'plan.md')==frozen['plan_sha256']
    assert '100% tests passed, 0 tests failed out of 51' in (build/'tests.log').read_text(encoding='utf-8')
    ...
    identity=dict(source_commit=frozen['source_commit'],binary_sha256=sha(build/'ExactEBRP.exe'),
        qualification_sha256=sha(OUT/f'qualification_{version}.json'),source=frozen['source'],
    ...
    try:
        for p in selected:
            for arm,preset in [('DS-X','research-round71-vds-interroute-descent'),('JI','research-round73-vds-joint-insertion')]:
                ...
                if arm=='JI':detail=replay_joint(p,dest,result)
                else:
