...
import analyze_round68
import round68_start_audit
round67_witness_model_audit=round68_start_audit.mapping

def main():
    assert not (run.OUT/'active_run.lock').exists(),'Do not package models while an optimizer queue is active'
    analyze_round68.analyze()
    round67_witness_model_audit.main()
    round68_start_audit.main()
    ...
    resource['offline_actual_start_audit_wall_seconds']=run.read(run.OUT/'start_checks.json')['offline_wall_seconds']
    resource['offline_actual_start_audit_optimizer_calls']=0
    ...
    for e in run.runner.entries():
        ...
        files=['launch.json','completion.json','phases.csv','heuristic.csv','hga_events.csv','ub_events.csv',
        ...
        prefixes=['gurobi_','hga_','external_gini_tree_','round65_','round66_','round67_','round68_']
        ...
        for original in sorted(source.glob('external/**/*.round68.start.*')):
            dest=destination/'starts'/original.name
            dest.parent.mkdir(parents=True,exist_ok=True)
            if original.suffix=='.csv':
                dest=dest.with_suffix('.csv.gz');dest.write_bytes(gzip.compress(original.read_bytes(),mtime=0))
                assert gzip.decompress(dest.read_bytes())==original.read_bytes()
            else:shutil.copyfile(original,dest)
            manifest.append(dict(number=e['charged_number'],artifact=str(dest.relative_to(run.OUT)),
                source=str(original.relative_to(run.ROOT)),source_sha256=run.sha(original),sha256=run.sha(dest),bytes=dest.stat().st_size))
            if original.name.endswith('.round68.start.json'):
                native=original.with_name(original.name.removesuffix('.round68.start.json'))
                logdest=destination/'starts'/(native.name+'.gz')
                logdest.write_bytes(gzip.compress(native.read_bytes(),mtime=0))
                assert gzip.decompress(logdest.read_bytes())==native.read_bytes()
                manifest.append(dict(number=e['charged_number'],artifact=str(logdest.relative_to(run.OUT)),
                    source=str(native.relative_to(run.ROOT)),source_sha256=run.sha(native),sha256=run.sha(logdest),bytes=logdest.stat().st_size))
    ...
    run.write(run.OUT/'analysis_identity.json',dict(analysis=run.sha(Path(analyze_round68.__file__)),
        packaging=run.sha(Path(__file__)),actual_start_audit=run.sha(Path(round68_start_audit.__file__)),physical_witness_verifier=run.sha(run.ROOT/'scripts/analyze_round61.py'),
        witness_model_mapping=run.sha(Path(round67_witness_model_audit.__file__)),
        reproduction=run.sha(run.ROOT/'scripts/round68_reproduce.py'),
