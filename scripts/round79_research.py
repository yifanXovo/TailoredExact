"""Round79 frozen six-role BDS-C complete-method validation on R78 production bytes."""
import json
import os
...
STAGE=ROOT/'results/unified_exact_round79'
ROLE_IDS=['E7','S12','N12','D3','C2','D4']
CAPS=dict(E7=120,S12=120,N12=120,D3=300,C2=300,D4=300)
...
def main():
    ...
    assert subprocess.check_output(['git','branch','--show-current'],cwd=ROOT).decode().strip()=='codex/round79-bdsc-small-validation'
    assert not subprocess.check_output(['git','status','--porcelain','--','src','include','tests','CMakeLists.txt','scripts/round79_research.py','results/unified_exact_round79/plan.md'],cwd=ROOT).strip(), 'Freeze measured source and driver before launch'
    ...
    assert read(STAGE/'prior_publication_verified.json')['head_sha']=='d064fe38c14caf7f8bcc201212cd7654e99f52c1'
    ...
    identity = dict(source_commit=frozen['source_commit'],binary_sha256=sha(build/'ExactEBRP.exe'),
        ...
        launches=launches,references=references,maximum_process_seconds=3780,formal_performance=True)
    ...
    try:
        for launch in launches:
            ...
            assert sum(r['audit'].get('native_calls_started',0) for r in records)<=240, 'Campaign resource ceiling; never an internal solver decision'
    finally:
        lock.unlink()
