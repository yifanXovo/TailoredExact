#!/usr/bin/env python3
"""Run selected contemporary Round 41 reference rows."""
...
ARMS = ("external-k4", "external-k1", "p-grb")
...
def run_one(instance_id: str, arm: str, process_cap: float,
    ...
    if arm == "p-grb":
        command = common.fair_pgrb_command(item, run_dir, process_cap)
    else:
        command = common.fair_c6_command(item, run_dir, process_cap)
        if arm == "external-k1":
            command.extend(("--round40-c6-coarse-start", "k1-single"))
    record: dict[str, Any] = {
        "schema": "round41-reference-run-v1",
        "round_id": 41,
        "run_id": run_id,
    ...
    if return_code != 0 or timed_out or not result_path.is_file():
        raise RuntimeError(f"Round 41 reference row failed: {run_id}")
    return common.load_json(result_path)
...
def main() -> int:
    ...
    parser.add_argument("--arm", action="append", choices=ARMS,
                        required=True)
    parser.add_argument("--process-cap", type=float, default=900.0)
